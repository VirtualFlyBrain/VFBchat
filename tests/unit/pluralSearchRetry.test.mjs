...
test('when the singular finds nothing either, the term abstains as before', async () => {
  ...
  // The ladder, then the spelling probes (the name with each candidate word
  // removed, longest first — see spellingRung.test.mjs), then one LOOSE search
  // per content word ("system" is a category word and "neurons" the noun, so
  // only "visual") to find terms VFB does hold that the answer can suggest
  // rephrasing around.
  assert.deepEqual(deps.calls.searches, [
    'visual system neurons', 'visual system neuron',
    'visual system', 'system neurons', 'visual neurons',
    'visual'
  ])
