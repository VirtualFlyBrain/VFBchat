...
## v4.2.21
- - **A long answer no longer dies silently on its way to the reader.** The stream can go minutes without a byte — a planner round on a busy gateway, a cold class-connectivity aggregation — and every hop between the server and the browser reads that silence as a dead connection: Node's own fetch gives up after 300 s between chunks, and proxies are often shorter. The connection now carries a keepalive every 15 seconds while nothing else is being sent. It is an SSE comment, so the chat window, the task battery and any other client ignore it; what changes is that nothing in between hangs up. Four battery questions had been cut off between five and seven and a half minutes in, none of them near the timeout.

  - **Non-English answers are linked again.** The answer is written in English, checked, and translated last — every gate and every linker reads English — but nothing had actually told the synthesiser that, and on a Japanese question it wrote the draft in Japanese about one run in three. The linkers then found no English name to link and the translation step translated Japanese into Japanese, so a correct answer reached the reader with no links in it at all. The instruction is now explicit on those turns, and names the question's language.

  - **A term named in the plural is linked.** "Mushroom bodies are paired neuropils" carried no link, because the name was matched against VFB's label letter for letter. The linker now also accepts a name's regular plural and a capital at the start of a sentence, and keeps the wording as the answer wrote it. A symbol's case is unchanged — "PN" still matches "PNs" and never "pn".

  - **An answer always leaves something to click.** A follow-on chip offers something new, so a query the answer was already built from is held back. That is right until it is all of them: the olfactory system holds exactly two, and when the check that runs before VFB is said to lack data ran both — about one welcome-screen answer in three — every chip was held back and the answer arrived with nothing under it. The chips come back in that case, because clicking one opens the rows themselves where the answer only summarised them.

  The task battery passes 78/78 against the live gateway, the first fully clean run: every tier, and all six language tasks (Persian in Latin script, Hungarian, Japanese, Chinese, German and the language-switch conversation). It is also the first run measured on the model this build is measured on — CI had been grading a model production does not run, which is what let the language regressions through. English turns are unchanged. [#70](https://github.com/VirtualFlyBrain/VFBchat/pull/70), [#71](https://github.com/VirtualFlyBrain/VFBchat/pull/71), [#72](https://github.com/VirtualFlyBrain/VFBchat/pull/72).

## v4.2.20
- - **A class symbol resolves to the class, and a symbol two classes share asks which one.** "What cell types are downstream of KCg?" answered about one hemibrain cell, KCg-d_R (FlyEM-HB:1003837184), and listed its 221 partners as the cell types downstream of KCg ([#66](https://github.com/VirtualFlyBrain/VFBchat/issues/66)). VFB's index ranks hundreds of `KCg-*` individuals above the class whose symbol is "KCg", so the class never reached the rows the resolver saw and a token-overlap guess took the first individual. A name that matches no document exactly but matches a class exactly now lifts to that class. And where the symbol is carried by more than one class — `KCg` names both `adult gamma Kenyon cell` and `gamma Kenyon cell` — the question decides if it names a stage; failing that the model is asked whether the question settles it and told not to guess; failing that the chat asks *"KCg" matches more than one VFB term: adult gamma Kenyon cell or gamma Kenyon cell. Which did you mean?* with a chip for each reading that re-asks the question about that term. Same-named individuals keep the existing answer-one-and-disclose-the-rest behaviour.

  - **An empty class-connectivity result is checked once before it is believed.** With the class resolved, the same question still said VFB holds no downstream connectivity for gamma Kenyon cells: the cache in front of VFBquery is still serving the empty results the pre-1.22.51 backend emitted for failed aggregations, under the URL shapes the MCP uses, while the plain URL returns 3,886 rows. A `DownstreamClassConnectivity` or `UpstreamClassConnectivity` count of 0 is now recomputed once past the cache on the same per-request budget as the existing count -1 retry; a second zero is believed. New log line: `[VFBchat] CLASS CONNECTIVITY ZERO RETRY | query_type= | id= | count_after= | ms=`.

  - **Roll-up classes are linked.** The class-partner summary registered only the ranked partners, so the roll-up classes named in its own sentence ("adult neuron", "adult CNS neuron", "adult interneuron") were the one set of names in an answer without a link. They and the self-connection row are linked now, in every language.

  Unit suite 1,387/1,389 (two skips). Verified live: "what cell types are downstream of KCg?" asks which gamma Kenyon cell class was meant, and choosing `adult gamma Kenyon cell` answers with 3,370 downstream classes ranked by synapses per connected pair; "strongest downstream partner cell types of EPG neurons" still resolves without a question; a Korean DNp32 answer links all 13 names. [#69](https://github.com/VirtualFlyBrain/VFBchat/pull/69).

## v4.2.19
- - **The answer comes back in the language the question was asked in.** At the NeuroFly table (10 September) a Persian question typed in Latin letters — "chand ta driverline dar flywire data vojood dare?" — got an English clarifying question back; "sorold fel az ellipszis testet jelolo GAL4 torzseket" was answered in Hungarian only because it ended "válaszolj magyarul"; and "can you reply in persian?" was answered "yes, I can". Nothing in the harness knew what language a message was written in. The planner now reads it (romanised Persian counts as Persian, and so on for any language), a typed question is answered in its own language, clarifying questions included, an explicit request ("reply in Persian", "válaszolj magyarul") pins the rest of the conversation, and a clicked follow-on keeps the conversation's language instead of dropping back to English. Right-to-left scripts align correctly in the chat window.

    **Every check still runs, because the translation is the last step.** The Hungarian answer had said VFB holds 327 driver lines above a table whose own link said 392, with no links in the prose — the absence gate, the count repair, the grounding audit and the term linkers all read English, and a Hungarian sentence went past every one of them. So the answer is still written in English, silently, every gate runs on it, and only then is the finished markdown rendered in the user's language, streamed. A deterministic check requires every link, every number (in any Unicode digit script) and every VFB identifier to survive the translation; a translation that drops one is retried once with the misses named, and then the English answer is shown with a note. An unverified translation is never shown. The English names of VFB terms stay in the text, with the local name in brackets on first mention, because they are what the links open. The same Hungarian question now answers with [392](https://v2.virtualflybrain.org/org.geppetto.frontend/geppetto?q=FBbt_00003678,TransgeneExpressionHere "Run in VFB: Transgene expression in ellipsoid body"), linked, and chips in Hungarian.

    **Names in another language resolve.** VFB's labels are English, so the planner writes anatomical and cell-type names as their standard English name ("ellipszis test" → "ellipsoid body"; symbols, gene names, line names and dataset names untouched), and the resolver gained a translation rung as the backstop for a name copied as written.

    **"Can you reply in Persian?" re-renders the previous answer** — links and chips included — in Persian, and pins the conversation, rather than answering the question about itself. Chip labels are localised; the query behind each chip stays English.

    English turns are byte-identical to v4.2.18. Six battery tasks across Persian in Latin script, Hungarian, Japanese, Chinese, German and the language-switch conversation pass against the live gateway, with new deterministic checks on script, English-stopword ratio, links and the carried language; unit suite 1,373/1,373. Each non-English turn costs one extra model call for the translation (9–84 s measured, scaling with the number of links); the bare language request still pays one planner round. New log line: `[VFBchat] TRANSLATION | language= | kind= | ok= | attempts= | links= | chars= | ms=`. [#68](https://github.com/VirtualFlyBrain/VFBchat/pull/68), from the Cologne feedback transcripts, 10 September.
