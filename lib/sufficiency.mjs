...
export function ledgerIsThin(ledger) {
  ...
  // An empty shelf used to return false — "not thin", i.e. sufficient. That is
  // an inference of adequacy from ignorance: no shelf means no term resolved a
  // query catalogue, which is the LEAST informed the ledger ever gets, not the
  // most. The honest verdict is thin. It costs nothing to say so, because
  // shouldCheckSufficiency's last gate is unrunQueries(), which reads the same
  // digests and is necessarily empty here — so no extra model call can fire on
  // this branch. It matters only for callers who ask this predicate what it
  // literally says, and to them it should not be lying.
  if (!shelf.length) return true
...
export async function maybeInjectSufficiencyQueries(ledger, deps, models = {}, log = () => {}) {
  ...
  try {
    res = await deps.callStructured({
      ...
      // Its own role since v4.0.0: this is a judgement call ("is what we have
      // enough to answer?") made once per question, so it gets the planner's
      // reasoning profile without being pinned to the planner's model override.
      model: models.sufficiency || models.planner
    })
  } catch (err) {
