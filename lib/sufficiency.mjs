...
/**
 * Term ids that an evidence-producing step actually looked at.
 *
 * Returns null when some step produced evidence without naming a term — a macro
 * tool, a search, a connectivity call keyed on something else. Those cannot be
 * attributed, so they count as covering the lot and the ledger is not thin.
 */
function lookedAtIds(ledger) {
  const answered = new Set((ledger?.evidence || []).map(e => e.stepId).filter(Boolean))
  if (!answered.size) return new Set()
  ...
  for (const s of ledger?.plan || []) {
    ...
    const id = s.args?.id
    if (!id) return null
    looked.add(String(id))
  }
  // Evidence from a step we cannot find is evidence all the same: treat it the
  // same as an unattributable one rather than as nobody having looked.
  return matched ? looked : null
}

/**
 * The ledger holds nothing a planned step produced — for at least one term that
 * still has queries on the shelf.
 *
 ...
 * only that is exactly the case where a false denial gets written.
 *
 * The per-term part matters for the two-term questions that motivated this.
 * "Which neurons in the mushroom body are cholinergic?" resolves both terms, the
 * intent router finds an unambiguous query for cholinergic neuron and none for
 * the region, and one satisfied step is enough to make a whole-ledger test read
 * "fed" — leaving the region's 366/304 unqueried and then denied. A term nobody
 * asked anything about is still a term nobody asked anything about.
 */
export function ledgerIsThin(ledger) {
  const ev = ledger?.evidence || []
  if (!ev.length) return true
  const looked = lookedAtIds(ledger)
  if (looked === null) return false
  if (!looked.size) return true
  return unrunQueries(ledger).some(c => !looked.has(String(c.id)))
}
