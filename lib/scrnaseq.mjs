...
/**
 * What a cluster IS, read off VFB's cluster name. The names are systematic —
 *   scRNAseq_2018_Davie_FULL_seq_clustering_gamma_Kenyon_cells
 *   scRNAseq_2022_FCA_FEMALE_HEAD_seq_clustering_gamma_Kenyon_cells
 *   scRNAseq_2023_AFCA_D30_MALE_FULL_seq_clustering_gamma_Kenyon_cells
 * — year, study, an optional condition (D30: 30-day-old), sex, tissue scope.
 * Those are the things that differ between the columns of an expression
 * table for one cell type, and none of them was in the header (#58): four
 * columns all read "gamma Kenyon cell".
 */
const CLUSTER_NAME_RE = /^scRNAseq_(\d{4})_([A-Za-z0-9]+?)(?:_(D\d+|L\d+|P\d+|[A-Z]\d+[A-Z]*))?(?:_(FEMALE|MALE|MIXED))?(?:_(FULL|HEAD|ANT|BODY|VNC|BRAIN|OL|CNS))?_seq_clustering_(.+)$/i
const TISSUE_WORDS = { FULL: 'whole fly', HEAD: 'head', ANT: 'antenna', BODY: 'body', VNC: 'VNC', BRAIN: 'brain', OL: 'optic lobe', CNS: 'CNS' }
export function describeCluster(name = '') {
  const m = String(name || '').match(CLUSTER_NAME_RE)
  if (!m) return null
  const [, year, study, cond, sex, tissue, cell] = m
  return {
    study, year, condition: cond || '', sex: sex ? sex.toLowerCase() : '', tissue: tissue ? (TISSUE_WORDS[tissue.toUpperCase()] || tissue.toLowerCase()) : '',
    cell: cell.replace(/_/g, ' ')
  }
}
/** "FCA 2022 · female head", "Davie 2018 · whole fly", "AFCA 2023 D30 · male whole fly". */
export function clusterDatasetLabel(name = '') {
  const d = describeCluster(name)
  if (!d) return ''
  const scope = [d.sex, d.tissue].filter(Boolean).join(' ')
  return `${d.study} ${d.year}${d.condition ? ` ${d.condition}` : ''}${scope ? ` · ${scope}` : ''}`
}

// A column header names what distinguishes the column: the cell type only
// when the columns differ in cell type (subtypes of the queried type), and
// always the dataset — study, year, sex and tissue — because those are what
// differ between the FCA female-head column and the FCA male-body column.
function shortCellType(perClusterEntry) {
  return String(perClusterEntry.cellType || '').replace(/^adult\s+/i, '').trim()
}
function columnLabel(perClusterEntry, { showCell }) {
  const dataset = clusterDatasetLabel(perClusterEntry.clusterName || '')
  const cell = shortCellType(perClusterEntry)
  if (showCell && cell && dataset) return `${cell} — ${dataset}`
  if (dataset) return dataset
  if (cell) return cell
  const name = String(perClusterEntry.clusterName || perClusterEntry.clusterId || '')
  const m = name.match(/clustering_(.+)$/i)
  return (m ? m[1] : name).replace(/_/g, ' ').trim()
}
...
export function renderExpressionMarkdown(matrix, termLabel = '') {
  ...
  for (const g of matrix.genes) {
    for (const p of (g.perCluster || [])) {
      if (!seen.has(p.clusterId)) { seen.add(p.clusterId); entries.push(p) }
    }
  }
  if (entries.length === 0) return ''
  const cellTypes = new Set(entries.map(shortCellType).filter(Boolean))
  const showCell = cellTypes.size > 1
  for (const p of entries) cols.push({ id: p.clusterId, label: columnLabel(p, { showCell }) })
  // Two clusters can still share a label (same study, sex and tissue, two
  // clusterings); number them rather than leave the reader guessing.
  const counts = new Map()
  for (const c of cols) counts.set(c.label, (counts.get(c.label) || 0) + 1)
  const seenLabel = new Map()
  for (const c of cols) {
    if ((counts.get(c.label) || 0) > 1) { const n = (seenLabel.get(c.label) || 0) + 1; seenLabel.set(c.label, n); c.label = `${c.label} (${n})` }
  }
  ...
  const scope = Array.isArray(matrix.requested_functions) && matrix.requested_functions.length && !(matrix.requested_genes || []).length
    ...
    : ''
  const single = cellTypes.size === 1 ? [...cellTypes][0] : ''
  const columnsNote = showCell
    ? 'columns are clusters: cell type — study, year, sex and tissue'
    : `columns are the ${single || termLabel || 'cell type'} clusters in each dataset: study, year, sex and tissue`
  const title = `**scRNA-seq expression${termLabel ? ` — ${termLabel}` : ''}${scope}** (${columnsNote}; expression level, % = fraction of cells expressing)`
