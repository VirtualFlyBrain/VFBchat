...
import { ranQueries } from './reproduce.mjs'
import { sanitizeLang } from './language.mjs'
...
// The kinds a term may claim to be when it arrives back from the client. A
// value outside this set is dropped, not repaired.
const KINDS = new Set(['individual', 'class', 'region', 'template', 'dataset'])
const MAX_PARENTS = 3
...
export function buildTurnContext(ledger, { maxTerms = MAX_TERMS, maxQueries = MAX_QUERIES_PER_TERM } = {}) {
  ...
  // The (id, query_type) pairs this turn actually dispatched — the same source
  // the Python reproduction is rendered from, so the two can never disagree.
  const ranTurn = ranQueries(ledger)
  const ranHere = new Set(ranTurn.map(r => `${r.id}::${r.query_type.toLowerCase()}`))
  ...
  for (const [name, t] of Object.entries(ledger?.terms || {})) {
    ...
    const queries = (t.digest?.queries || [])
      ...
      .map(q => ({
        ...
        // Did THIS conversation actually run it? Counts deliberately do not
        // travel, because a count is a fact with an age — but "we ran this" does
        // not go stale, and it is what lets a later turn hand back the working
        // for a result an earlier turn produced.
        ran: ranHere.has(`${t.id}::${String(q.query_type).toLowerCase()}`)
      }))
      .slice(0, maxQueries)
    // A query the turn ran that the digest never advertised would otherwise be
    // forgotten. Keep it: it is the strongest possible evidence the query works
    // for this term, since we just ran it.
    for (const r of ranTurn) {
      if (r.id !== t.id) continue
      if (queries.some(q => q.query_type.toLowerCase() === r.query_type.toLowerCase())) continue
      if (queries.length >= maxQueries) break
      queries.push({ query_type: r.query_type, label: '', count: -1, countKind: 'unknown', ran: true })
    }
    terms.push({
      name: clip(name),
      label: clip(t.digest?.name || idLabel[t.id] || name),
      id: t.id,
      // Whether this is one specific example or the type itself decides which
      // questions can be asked of it at all, and it was not carried — so on a
      // follow-up turn about an already-resolved term every kind predicate in
      // the orchestrator silently read false, and an individual was treated as
      // a class. `parents` is how a type-level follow-up ("what is it
      // downstream of?") reaches the class that can answer it.
      kind: KINDS.has(t.kind || t.digest?.kind) ? (t.kind || t.digest?.kind) : undefined,
      parents: (t.parents || t.digest?.parents || [])
        .filter(p => p?.id && VFB_ID_RE.test(p.id))
        .slice(0, MAX_PARENTS)
        .map(p => ({ id: p.id, label: clip(p.label || p.id, 80) })),
      queries
    })
  ...
  return { v: CONTEXT_VERSION, terms, registry, lang: sanitizeLang(ledger?.langContext) }
}
...
export function sanitizeContext(raw) {
  const empty = { v: CONTEXT_VERSION, terms: [], registry: [], lang: null }
  ...
  // The conversation's language: what the last typed turn was written in, or
  // what the user asked for ("reply in Persian" pins it). Validated like the
  // rest — a code reaches a prompt, so it has to be a code.
  const lang = sanitizeLang(raw.lang)
  ...
  for (const t of (Array.isArray(raw.terms) ? raw.terms : [])) {
    ...
    for (const q of (Array.isArray(t.queries) ? t.queries : [])) {
      ...
      queries.push({
        ...
        // Strictly a boolean: this claim becomes a line of code we tell the user
        // reproduces their answer, so a truthy string must not be able to set it.
        ran: q.ran === true
      })
    ...
    const kind = KINDS.has(t.kind) ? t.kind : undefined
    const parents = []
    for (const p of (Array.isArray(t.parents) ? t.parents : [])) {
      const pid = typeof p?.id === 'string' ? p.id.trim() : ''
      if (!VFB_ID_RE.test(pid) || parents.some(x => x.id === pid)) continue
      parents.push({ id: pid, label: clip(p.label || pid, 80) })
      if (parents.length >= MAX_PARENTS) break
    }
    terms.push({ name, label: label || name, id, kind, parents, queries })
  ...
  return { v: CONTEXT_VERSION, terms, registry, lang }
}
...
export function mergeContext(prev, turn, { maxTerms = MAX_TERMS, maxRegistry = MAX_REGISTRY } = {}) {
  ...
  for (const t of [...b.terms, ...a.terms]) {
    ...
    // `ran` is cumulative over the conversation, not a property of this turn: the
    // fresher catalogue wins on labels and counts, but a query an earlier turn
    // ran was still run. Anything the fresher entry dropped entirely is carried
    // back in, so the working never shrinks as the conversation goes on.
    const older = a.terms.find(o => o.id === t.id)
    if (older && t.queries.length) {
      const ranBefore = new Set(older.queries.filter(q => q.ran).map(q => q.query_type.toLowerCase()))
      const queries = t.queries.map(q =>
        ranBefore.has(q.query_type.toLowerCase()) ? { ...q, ran: true } : q)
      const present = new Set(queries.map(q => q.query_type.toLowerCase()))
      for (const q of older.queries) {
        if (!q.ran || present.has(q.query_type.toLowerCase())) continue
        if (queries.length >= MAX_QUERIES_PER_TERM) break
        queries.push(q)
      }
      terms.push({ ...t, queries })
      if (terms.length >= maxTerms) break
      continue
    }
    terms.push(t)
  ...
  // This turn's language decision wins; a turn that made none (an older
  // server, a result that never reached the harness) keeps the conversation's.
  return { v: CONTEXT_VERSION, terms: terms.slice(0, maxTerms), registry, lang: b.lang || a.lang }
}
...
/**
 * The query catalogue a previous turn established for an id.
 *
 * This used to say it validated a follow-on chip's `{id, query_type}` before the
 * chip was run. It did not: it had a unit test and no production call site, and
 * a client-authored chip whose halves passed detectFocusPlan's two regexes was
 * dispatched unchecked. It could not have been that gate either — the carried
 * catalogue is truncated to MAX_QUERIES_PER_TERM, so a query's absence from it
 * is not evidence that the term lacks it, and rejecting on that would refuse
 * legitimate chips.
 *
 * The real gate is dropUnofferedTemplateSteps in lib/orchestrator.mjs, which
 * checks a chip-origin step against VFB's own catalogue after resolution. This
 * function is what the answer's follow-on chips are built from, and nothing more.
 */
