...
export async function sendStructuredTelemetry(event) {
  ...
  // What leaves the University is durations, counts and outcome types — nothing
  // derived from the content of a question or an answer.
  //
  // `topic_category` (classified from the question) and `vfb_term_ids` (the
  // ontology terms the answer mentioned) were removed deliberately. Both are
  // question-derived content, and sending them made "aggregate analytics" the
  // wrong description of a per-request event stream leaving the country. They
  // are still recorded in the University-hosted analytics counters above, which
  // is where the reach reporting is actually done.
  const payload = {
