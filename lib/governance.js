...
export async function sendStructuredTelemetry(event) {
  ...
  // The 7 September check-in: real chat traffic had run all day and GA4 showed
  // one event — a synthetic one sent by hand to prove the credentials and the
  // payload shape were fine. They were. What was never fine, and never visible,
  // is whatever this fetch actually does on the production host: this catch
  // swallowed every outcome — a DNS block on google-analytics.com specifically
  // (the container reaches OpenAI, ELM and NCBI without trouble), a non-2xx from
  // Google, anything — with nothing written anywhere, ever. A telemetry pipeline
  // that fails silently is not degraded, it is off, and nobody can tell the
  // difference between "no traffic" and "no delivery" without instrumenting the
  // one place that already knows which it is.
  //
  // Logged: the response status, or the error's name. Never the endpoint (its
  // query string carries GA_API_SECRET) and never `err.message` — some runtimes
  // fold the request URL into it — so a fetch failure cannot leak the secret it
  // failed to authenticate with. This is operational status about our own
  // outbound call, not a user's text, so it is outside consoleLeakGuard's remit
  // (see tests/unit/consoleLeakGuard.test.mjs) the same way every other
  // structural console.error in this file is.
  try {
    ...
    if (!res.ok) {
      console.error(`[VFBchat] GA telemetry rejected: HTTP ${res.status}`)
    }
  } catch (err) {
    console.error(`[VFBchat] GA telemetry unreachable: ${err?.name || 'fetch failed'}`)
  }
}
