...
import { buildReproduction, reproductionBlock } from './reproduce.mjs'
...
export async function runLiveHarness(opts) {
  ...
  const reproduction = buildReproduction(r.ledger || {}, { question: opts.question || '' })
  return {
    answer: (r.answer || '') + reproductionBlock(reproduction, opts.question || ''),
