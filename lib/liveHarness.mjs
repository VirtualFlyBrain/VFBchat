...
import { planVoteKey, votePlanWithEscalation, acceptBareLanguageRequest } from './planner.mjs'
...
import { buildTables, galleryThumbnails, termExampleThumbnails } from './resultTables.mjs'
...
export function buildLiveDeps(p) {
  ...
  const callTextStream = ({ messages, model, sourceQuotes, silent }) => {
    ...
    return p.streamText({
      ...
      // Accumulate without emitting: the English draft of a non-English turn.
...
export async function runLiveHarness(opts) {
  ...
  const followOns = buildFollowOns(r.ledger || {}, { countEstimates: collected.countEstimates })
  ...
  return {
    ...
    // The language this turn is answered in (lib/language.mjs), and whether the
    // turn was a bare "reply in X" — nothing looked up, the previous answer to
    // be re-rendered by the caller, which holds the history.
    language: r.ledger?.language || 'en',
    languageSwitch: Boolean(r.languageSwitch),
    complete: Boolean(r.complete),
    ledger: r.ledger,
    trace: r.trace,
    // Deterministic displays the orchestrator built from ranked data come
    // first: they are the ones whose order the prose describes.
    graphs: [...(r.ledger?.graphs || []), ...collected.graphs],
    thumbnails: collected.thumbnails,
    expression: collected.expression,
    appendices: r.ledger?.appendices || [],
    ...
    tables: [...(r.ledger?.tables || []), ...buildTables(r.ledger || {}, opts.question || '')],
    galleryThumbnails: galleryThumbnails(r.ledger || {}, opts.question || ''),
    termThumbnails: termExampleThumbnails(r.ledger || {}),
