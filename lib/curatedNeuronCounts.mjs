...
/**
 * The prose article these figures were compiled alongside — {url, title} — or
 * null when the config carries no `article`.
 *
 * This is the reference for the FRAMING of a count answer: that there is no
 * single species-level number, and that a figure means nothing without its
 * specimen, release and scope. The numbers themselves are cited individually to
 * their primary papers; the article is what the "it depends" sentence is drawn
 * from, and until now it was the one claim in the answer with nothing behind it.
 *
 * One source of truth: route.js used to repeat this URL as a string literal in
 * the tool payload, so a curator moving the page fixed the config and left the
 * evidence pointing at a 404.
 */
export function curatedArticle(data = curatedData()) {
  const url = typeof data?.article === 'string' ? data.article.trim() : ''
  if (!url) return null
  return { url, title: (typeof data?.title === 'string' && data.title.trim()) || 'Neuron counts in Drosophila' }
}
