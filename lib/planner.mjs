...
GRAPHS / CONNECTIVITY: when the user asks what an entity connects to, its connectivity partners/inputs/outputs, or explicitly for a connectivity "graph"/"network"/"in graph form", plan a vfb_find_connectivity_partners step with endpoint_type set to the entity's natural-language name and direction "downstream" (for outputs/targets/"connects to") or "upstream" (for inputs/sources). That step returns ranked partner edges which the UI renders as a graph automatically — do not plan create_basic_graph yourself. Still list the entity in terms_to_resolve.
