...
GRAPHS / CONNECTIVITY: when the user asks what an entity connects to, its connectivity partners/inputs/outputs, "summarised connectivity", or explicitly for a connectivity "graph"/"network"/"in graph form", plan a vfb_find_connectivity_partners step. The endpoint_type is the entity's natural-language name and may be a NEURON TYPE (e.g. giant fiber neuron) OR a BRAIN REGION (e.g. medulla, mushroom body). Set direction "downstream" (for outputs/targets/"connects to"/"from X") or "upstream" (for inputs/sources/"to X"). This is the route for "(class summarised) connectivity from/of <region>" too — do NOT answer those only from term-info counts. The step returns ranked partner edges which the UI renders as a graph automatically — do not plan create_basic_graph yourself. Still list the entity in terms_to_resolve.
