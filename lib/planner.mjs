 ...
 * @param {function} o.sample         async (k) => Array<plan>  — the raw generations
 ...
 * @param {object}   o.policy         {minAgreement, extraVotes, maxRounds}
 * @param {function} [o.onContested]  called once, before the extra round is bought
 * @returns {Promise<{ok, value, agreement, votes, escalated, rounds}>}
 */
export async function votePlanWithEscalation(o = {}) {
  ...
  let pool = (await o.sample(Math.max(1, o.votes || 1))) || []
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    const more = (await o.sample(extraVotes)) || []
