...
GRAPHS / CONNECTIVITY: when the user asks what a NEURON TYPE connects to, its connectivity partners/inputs/outputs, or explicitly for that neuron's connectivity "graph"/"network"/"in graph form", plan a vfb_find_connectivity_partners step with endpoint_type set to the neuron type's natural-language name (e.g. giant fiber neuron, MBON, Mi1) and direction "downstream" (for outputs/targets/"connects to") or "upstream" (for inputs/sources). It returns ranked partner edges the UI renders as a graph automatically — do not plan create_basic_graph yourself. This applies to NEURON TYPES only: for a BRAIN REGION (medulla, mushroom body, antennal lobe, …) use vfb_get_term_info and its query counts instead — region-level class-connectivity edges are not available, so do not route a region to vfb_find_connectivity_partners. Still list the entity in terms_to_resolve.
...
function cleanSubject(s = '') {
  return String(s)
    ...
    .replace(/\b(in|of|the)\s+(the\s+)?(drosophila|fly|adult|larval)\b.*$/i, '') // drop "in the Drosophila ..."
    .replace(/[?.!,;:]+$/, '')
    .replace(/\s+/g, ' ')
    .trim()
}
