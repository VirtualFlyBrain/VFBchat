...
//
// The preposition can also come AFTER the noun — "which datasets they're in",
// "which dataset each comes from" — which is the phrasing the NeuroFly workshop
// published for its discovery exercise (issue #39). Both forms are a scope. The
// stranded form must not catch "what datasets are in VFB", which is the
// enumeration itself, so the preposition may not be followed by VFB.
const DATASET_SCOPE_RE = /\b(?:across|within|in|from|for|per|by|throughout|between|among)\s+(?:all\s+|each\s+|every\s+|the\s+|both\s+|these\s+|those\s+|any\s+)*(?:connectome\s+|em\s+)?data\s?sets?\b|\bdata\s?sets?\s+(?:\w+['’]?\w*\s+){0,3}?(?:in|from)\b(?!\s+(?:vfb|virtual))/i
const NAMED_SUBJECT_RE = /'[^']{2,}'|"[^"]{2,}"|\bVFB_[0-9a-z]+\b|\bFBbt_\d+\b|\b(?:neuron|cell|gene|transgene|region|body|lineage)\s+types?\b/i
// A subject can also be named without quotes or an id: a symbol-like token — one
// with a capital letter after its first character, or a digit — right before
// "neuron(s)" or "cell(s)" ("DA1 lPN neurons", "Kenyon cells", "LPLC2 neurons").
// Case-sensitive on purpose: a sentence-initial "List neurons" is not a symbol.
const SYMBOL_SUBJECT_RE = /\b(?:[A-Za-z0-9'/-]*(?:[A-Za-z0-9'/-][A-Z]|\d)[A-Za-z0-9'/-]*|[A-Z][a-z]{2,})\s+(?:neurons?|cells?)\b/
const namesASubject = (q) => NAMED_SUBJECT_RE.test(q) || SYMBOL_SUBJECT_RE.test(q.replace(/^\s*\w+/, ''))
...
export const PLAN_SCHEMA = {
  ...
  required: ['intent', 'underspecified', 'clarifying_question', 'terms_to_resolve', 'steps', 'language', 'requested_language', 'language_request_only'],
  properties: {
    ...
    language_request_only: { type: 'boolean' },
    steps: {
      type: 'array',
      items: {
        type: 'object',
...
- terms_to_resolve: anatomy / neuron / gene names mentioned that need resolving to VFB ids (use the user's natural-language names, not ids). When the current question refers back to an entity by pronoun, put that entity's full name here (taken from the prior conversation), not the pronoun. VFB's labels are English: when the question is written in another language, write each anatomical or cell-type name here as its standard ENGLISH name (Hungarian "ellipszis test" -> "ellipsoid body"; Persian "جسم قارچی" -> "mushroom body"; German "Kenyon-Zellen des Pilzkörpers" -> "Kenyon cell"), one entity per entry, and leave symbols, gene names, driver-line names and dataset names (GAL4, fru, FlyWire, hemibrain) exactly as written. Never put a whole phrase or a category word (driver lines, split-GAL4 lines, neurons, images) into an entry: "Welche split-GAL4-Linien markieren die Kenyon-Zellen" resolves "Kenyon cell", not "Kenyon-Zellen split-GAL4".
- language: the ISO 639-1 code of the language the user WROTE this message in (en, fa, hu, de, zh, …). Text in Latin letters can still be another language — Persian or Hindi typed in the Latin alphabet is fa or hi, not en. A message in English that merely contains foreign anatomical names is en.
- requested_language: the ISO 639-1 code of a language the message asks to be ANSWERED in ("reply in Persian", "válaszolj magyarul", "auf Deutsch bitte"), else "". This can differ from language ("can you reply in persian?" is written in en and requests fa).
- language_request_only: true only when the message asks for a language and asks nothing else — no VFB question, no entity, nothing to look up. Then also set steps to [] and terms_to_resolve to [].
...
export function buildPlannerMessages(question, toolCatalogue = [], history = [], context = null) {
  ...
  return [
    { role: 'system', content: PLANNER_SYSTEM + serviceIdentityBlock() },
...
export function normalizePlan(raw = {}, question = '') {
  ...
  // The language fields are read by lib/language.mjs (decideTurnLanguage). They
  // are normalised there, not here: this keeps the planner's raw reading so a
  // wrong code and a name-instead-of-code are both recoverable. A bare language
  // request is only honoured when the plan really is bare — a planner that
  // flags it while also planning a lookup has misread the message, and the
  // lookup is the safer half to keep.
  const language = typeof raw.language === 'string' ? raw.language.trim() : ''
  const requestedLanguage = typeof raw.requested_language === 'string' ? raw.requested_language.trim() : ''
  const terms = (Array.isArray(raw.terms_to_resolve) ? raw.terms_to_resolve.map(String) : [])
    .filter(Boolean).filter(n => !isServiceName(n) && !isLlmClientName(n))
  const languageRequestOnly = Boolean(raw.language_request_only) && Boolean(requestedLanguage) &&
    steps.length === 0 && terms.length === 0
  return {
    intent,
    // A bare language request is not underspecified: it is answered by
    // re-rendering the previous turn, never by asking what the user meant.
    underspecified: languageRequestOnly ? false : underspecified,
    clarifying_question: (underspecified && !languageRequestOnly) ? asked : '',
    terms_to_resolve: terms,
    steps,
    language,
    requested_language: requestedLanguage,
    language_request_only: languageRequestOnly
  }
}
...
export async function votePlanWithEscalation(o = {}) {
  ...
  // A reading the caller will take from round one without a vote. "Can you
  // reply in Persian?" is the case: the votes disagree about which tool to
  // reach for on a message that names nothing to look up, and the escalation
  // round buys three more opinions — five minutes of them, measured — about a
  // question that has no plan to disagree over. See acceptBareLanguageRequest.
  if (typeof o.accept === 'function') {
    const accepted = o.accept(pool)
    if (accepted) {
      return { ok: true, value: accepted, agreement: 1, votes: pool.length, escalated: false, budgetExhausted: false, rounds: [{ agreement: 1, votes: pool.length, escalated: false, accepted: true }] }
    }
  }
...
/**
 * The one reading a planner round may settle without a vote: the message is a
 * bare request for a language. Accepted when at least one vote read it that
 * way AND no vote found anything to resolve — a vote that extracted an entity
 * is evidence the message asked something, and then the votes are counted as
 * usual. Returns the accepted raw plan, or null.
 */
export function acceptBareLanguageRequest(pool = []) {
  const plans = (Array.isArray(pool) ? pool : []).map(raw => normalizePlan(raw, ''))
  if (!plans.length) return null
  if (plans.some(p => (p.terms_to_resolve || []).length > 0)) return null
  const i = plans.findIndex(p => p.language_request_only)
...
export function detectFocusPlan(question = '', focus = null) {
  ...
  return {
    ...
    clarifying_question: '',
    terms_to_resolve: [id],
    // `via_chip` marks this as CLIENT-AUTHORED. The two regexes above check only
    // that the id and the query type are the right shape; whether this term
    // actually offers that query is checked after resolution, against VFB's own
    // catalogue, in reconcileStepsWithOfferedQueries. It cannot be checked here:
    // the carried context is truncated, so absence from it is not evidence.
    steps: [{ id: 's1', tool: 'vfb_run_query', answers: [q], args: { id, query_type: queryType }, via_chip: true }]
  }
}
...
export function detectFastPath(question = '') {
  ...
  if (DATASETS_INTENT.test(q) && !DATASET_SCOPE_RE.test(q) && !namesASubject(q)) {
    return {
