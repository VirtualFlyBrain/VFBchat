...
export function detectFastPath(question = '') {
  ...
  if (!q) return null
  // "What datasets are available?" — list VFB's datasets deterministically instead
  // of letting the weak planner abstain (and the empty-VFB literature fallback pull
  // an irrelevant paper). The step's result is real VFB evidence, so no fallback.
  if (DATASETS_INTENT.test(q)) {
    return {
      intent: 'other', underspecified: false, clarifying_question: '', terms_to_resolve: [],
      steps: [{ id: 's1', tool: 'vfb_list_connectome_datasets', answers: [q], args: {} }]
    }
  }
