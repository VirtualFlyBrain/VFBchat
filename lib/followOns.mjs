...
import { buildReferenceSources, sourceKey, sourceTitle } from './referenceSources.mjs'
 ...
 * @param {object} ledger
 * @param {object} [opts]
 * @param {number} [opts.maxChips]
 * @param {Array}  [opts.countEstimates] parsed vfb_get_region_neuron_count results,
 *   which carry the reviewed article a curated figure is framed by and the
 *   primary paper behind each figure. They are tool results rather than ledger
 *   evidence rows, so they have to be handed in.
 * @returns {{ terms:Array, chips:Array, sources:Array }}
 *   chip = { kind:'ask'|'vfb', label, query?, url?, title }
 *   source = { kind:'vfb'|'doc'|'publication', label, url, id, title? }
 */
export function buildFollowOns(ledger, { maxChips = 6, countEstimates = [] } = {}) {
  const terms = []
  const chips = []
  const sources = []
  ...
  for (const [name, t] of resolved) {
    ...
    if (url && !seenSource.has(t.id)) {
      seenSource.add(t.id)
      // kind is what the reader is being sent to, and the Sources line words the
      // hover text from it. A term report, a documentation page and a paper are
      // three different promises about what is on the other side of the link.
      sources.push({ kind: 'vfb', label, url, id: t.id, superseded, title: sourceTitle({ kind: 'vfb', label, url }) })
    }
  ...
  for (const e of (ledger?.evidence || [])) {
    ...
    const url = e.url || e.locator?.url
    const key = sourceKey(url)
    if (!key || refKeys.has(key)) continue
    refKeys.add(key)
    const label = stripMarkdown(e.title || e.locator?.title || docLabelFromUrl(url))
    sources.push({ kind: 'doc', label, url, id: null, title: sourceTitle({ kind: 'doc', label, url }) })
  }

  // Everything else the answer stood on that is not a VFB term: the reviewed
  // article a curated figure is framed by, and the primary papers those figures
  // are quoted from. Deliberately last — a reader scanning Sources wants the
  // entities first — and deduped against the pages already listed above.
  for (const s of buildReferenceSources(ledger, countEstimates, { seen: refKeys })) {
    sources.push(s)
  }
...
export function linkifyKnownTerms(text, termLinks) {
  ...
  return parts.map((part, i) => {
    ...
    for (const t of termLinks) {
      ...
      //
      // The name as WRITTEN may be the plural: "the mushroom bodies are paired
      // neuropils" is how the synthesiser opens about the mushroom body about
      // one time in three, and the sentence then carried no link at all
      // (battery L3). The plural suffix is matched as part of the span and kept
      // in the link text, so the prose reads as written and the link target is
      // the singular term. Only a trailing s/es/ies is accepted; a name that
      // already ends in "s" gets no suffix.
      const named = writtenFormsPattern(t.name)
      const re = new RegExp(`(?<![\\w-])(?:\\[${named}\\]|(?<!\\[)${named})(?![\\w-])`)
      for (let s = 0; s < segs.length; s++) {
        ...
        // Link text is the span as the prose wrote it (case and plural
        // included), brackets stripped; the title names the term.
        const written = m[0].replace(/^\[|\]$/g, '')
        const url = `[${written}](${t.url} "${titleSafe('Open ' + t.name + ' in Virtual Fly Brain')}")`
...
/**
 * The regex alternation of the ways a term name is written in prose: the name
 * itself, its regular English plural, and either with the first letter
 * capitalised when the name starts lower-case (a sentence may open with it).
 * Only the first letter may differ in case — a symbol's case still counts, so
 * "PN" does not match "pn" — and a name already ending in "s" takes no plural.
 * Exported for the unit test.
 */
export function writtenFormsPattern(name = '') {
  const n = String(name)
  const forms = new Set([n])
  if (n.length >= 2 && !/s$/i.test(n)) {
    if (/[^aeiou]y$/i.test(n)) forms.add(n.slice(0, -1) + 'ies')
    else if (/(?:x|ch|sh)$/i.test(n)) forms.add(n + 'es')
    else forms.add(n + 's')
  }
  const cased = [...forms].map(f => (/^[a-z]/.test(f) ? `[${f[0].toUpperCase()}${f[0]}]${escapeRe(f.slice(1))}` : escapeRe(f)))
  // Longest first, so the plural wins over the singular prefix it contains.
  cased.sort((a, b) => b.length - a.length)
  return `(?:${cased.join('|')})`
}
