...
export function buildFollowOns(ledger, { maxChips = 6 } = {}) {
  ...
  for (const [name, t] of resolved) {
    ...
    const queries = (t.digest?.queries || []).filter(q =>
      ASK_TEMPLATES[q.query_type] && !(countKindOf(q) === 'exact' && !(q.count > 0)))
    queries.sort((a, b) => chipSortValue(b) - chipSortValue(a))
    for (const q of queries) {
      const query = ASK_TEMPLATES[q.query_type].replace('{term}', label)
      const key = `ask:${query.toLowerCase()}`
      if (seenChip.has(key)) continue
      seenChip.add(key)
      chips.push({ kind: 'ask', label: `${query}${chipCountSuffix(q)}`, query, title: `Ask VFB: ${query}` })
    }
  }
...
export function linkifyCounts(text, countLinks) {
  ...
  return parts.map((part, i) => {
    if (i % 2 === 1) return part
    return part.replace(/(?<![^\s(])(more than\s+|over\s+|at least\s+)?\b\d[\d,]*\b(?![\w\-[\]{}]|[.,]\S)/gi, (m, bound) => {
      if (bound) return m
