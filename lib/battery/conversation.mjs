...
/** The prose of an answer: links, code and URLs out, so scripts and words are counted on what a reader reads. */
function proseOf(answer = '') {
...
/**
 * Share of the answer's letters that belong to a Unicode script ("Arabic",
 * "Han", "Cyrillic", "Latin", …). Letters only — digits, punctuation and the
 * English entity names inside link text all sit outside the count of what the
 * translation was supposed to write, so the labels VFB keeps in English do not
 * count against a Persian answer.
 */
export function scriptShare(answer = '', script = 'Latin') {
  const prose = proseOf(answer).replace(/\[[^\]]*\]/g, ' ')
  const letters = prose.match(/\p{L}/gu) || []
  if (!letters.length) return 0
  let re
  try { re = new RegExp(`\\p{Script=${script}}`, 'u') } catch { return 0 }
  const hits = letters.filter(ch => re.test(ch)).length
  return hits / letters.length
}

// Function words that are English and not also a word of some other Latin-
// alphabet language: "a", "an", "in", "is", "as", "to" are all words in
// Hungarian, German, Dutch, Spanish or Italian and are left out.
const ENGLISH_STOPWORDS = new Set(['the', 'of', 'and', 'are', 'with', 'for', 'that', 'this', 'by', 'from', 'holds', 'does', 'not', 'which', 'these', 'those', 'has', 'have', 'its', 'be', 'was', 'were', 'currently', 'records'])

/** Share of the answer's words that are English function words — near zero in any other language. */
export function englishStopwordRatio(answer = '') {
  const all = proseOf(answer).replace(/\[[^\]]*\]/g, ' ').toLowerCase().match(/\p{L}+/gu) || []
  if (!all.length) return 0
  return all.filter(w => ENGLISH_STOPWORDS.has(w)).length / all.length
}
...
export function checkTurn(expect, observed = {}) {
  ...
  // The language checks. Deterministic by design: a script test where the
  // language has its own script, an English-stopword ratio where it shares the
  // Latin alphabet, and the links counted, because a translation that lost its
  // links is the failure the whole translate-last design exists to prevent.
  if (expect.answer_script) {
    const share = scriptShare(answer, expect.answer_script)
    if (share < 0.5) {
      problems.push(`answer is not written in the ${expect.answer_script} script (${Math.round(share * 100)}% of its letters are)`)
    }
  }
  if (expect.answer_not_english) {
    const ratio = englishStopwordRatio(answer)
    if (ratio > 0.06) {
      problems.push(`answer reads as English (${Math.round(ratio * 100)}% of its words are English function words)`)
    }
  }
  if (expect.context_lang) {
    const want = typeof expect.context_lang === 'string' ? { code: expect.context_lang } : expect.context_lang
    const got = observed.context?.lang || null
    if (!got || got.code !== want.code) {
      problems.push(`context language is ${got ? got.code : 'unset'}, expected ${want.code}`)
    } else if (typeof want.pinned === 'boolean' && got.pinned !== want.pinned) {
      problems.push(`context language ${got.code} is ${got.pinned ? '' : 'not '}pinned, expected ${want.pinned ? 'pinned' : 'not pinned'}`)
    }
  }
  if (Number.isFinite(Number(expect.min_links))) {
    const links = (answer.match(/\]\(https?:\/\//g) || []).length
    if (links < Number(expect.min_links)) {
      problems.push(`expected at least ${expect.min_links} link(s) in the answer, got ${links}`)
    }
  }
