...
// One TAB per level, not two spaces: three of the answer sanitisers collapse
// runs of spaces and tabs (`[ \t]{2,}` in lib/harnessFraming.mjs, grounding,
// countProvenance), which flattened every nested list to one level. A single
// tab is one character, survives them all, and is four columns of indentation
// to CommonMark — enough to nest under a "- " item at any depth.
const INDENT = '\t'
...
function renderList(list, depth, budget, lines) {
  for (const x of list) {
    ...
    lines.push(`${INDENT.repeat(depth)}- ${link}`)
...
export function renderHierarchyMarkdown(tree, { maxNodes = 150 } = {}) {
  ...
  if (tree.ancestors.length) {
    ...
    const lines = chain.map((x, i) => `${INDENT.repeat(i)}- [${x.label}](${VFB_REPORT}${x.id})`)
    lines.push(`${INDENT.repeat(chain.length)}- **${tree.label}**`)
