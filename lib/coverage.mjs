...
import { queryRelevanceScore, questionKindSets, listQueryWords, KIND_MATCH_WEIGHT } from './queryRelevance.mjs'
...
/**
 * Names the resolver tried and could not match to any VFB term.
 *
 * This is a fifth state and it does not belong on the shelf, because the shelf
 * is per-QUERY and these names have no queries — that is precisely what is
 * wrong with them. It lives here rather than beside the absence guard that
 * consumes it so that the shelf and the guard read the ledger through the same
 * function, and so coverage.mjs does not have to import a module that imports
 * it back.
 *
 ...
 * reader who never typed the name does not need an apology for a guess, and its
 * failure says nothing about whether an absence in the answer is licensed.
 */
export function unmatchedNames(ledger) {
  return Object.entries(ledger?.terms || {})
    ...
    .map(([name, t]) => ({
      ...
      // A ladder cut short for time established nothing about VFB's index
      // either — only that we stopped looking.
...
/**
 * Can this holding be stated as a NUMBER of things VFB holds?
 *
 * The sentence the prompt asks for is "VFB holds <count> <label> for <term>".
 * countKind 'unknown' means there is no count to put in the count slot, and the
 * model filled it with the only other token on the line: "VFB holds Neurons
 * connected to DA1_lPN for AL.MB_CA.83 (FlyWire:720575940630066007), available."
 * — the word "available" from countPhrase, stranded, because the same paragraph
 * forbids saying the rest of that phrase ("run this query for the count").
 * 'many' is fine: "more than 1,000" is a real complement. An exact zero is not a
 * holding at all.
 */
function countIsStatable(e) {
  const kind = e?.countKind || (Number(e?.count) < 0 ? 'unknown' : 'exact')
  if (kind === 'many') return true
...
/**
 * The subset of unanswered asks the answer may say OUT LOUD.
 *
 * unansweredAsks is deliberately generous, because it also feeds the sufficiency
 * pre-filter, where a lexical near-miss is a good reason to look again. SPEAKING
 * is a higher bar than LOOKING, and sharing one threshold between them is what
 * put a connectivity count on the tail of a similarity question, an image count
 * on the tail of a connectivity question, and an NBLAST count on the tail of a
 * connectivity question — nine of nineteen workshop answers at 3.9.1.
 *
 * `relevance > 0` is satisfied by ONE shared stemmed word. KIND_MATCH_WEIGHT is
 * added if and only if the query's semantic kind is one the question asked for,
 * so requiring at least that much IS the sentence "the question asked for this
 * kind of thing" — and it costs nothing where the kind does match, because the
 * lexical part is still added on top and still does the ranking.
 *
 * Filtered items are not lost: they stay in HELD, NOT YET RUN, where they are
 * still protected from denial and still offered to the user as a follow-on. They
 * simply lose the licence to be spoken in an answer that was not about them.
 */
function worthSaying(asks) {
  return asks.filter(e => e.relevance >= KIND_MATCH_WEIGHT && countIsStatable(e))
}
 ...
 */
export function renderShelf(shelf, { unmatched = [] } = {}) {
  ...
  const asks = worthSaying(unansweredAsks(shelf)).slice(0, WORTH_SAYING_CAP)
  ...
  //
  // Everything above is organised around terms that RESOLVED. When nothing
  // resolves, renderNoCoverageFloor takes over and states the prohibition in
  // full. The case neither of them covered is the one in between: a question
  // with two subjects where one matched and the other did not. The shelf is
  // non-empty, so the floor never fires; the unmatched name appears nowhere, so
  // nothing in the prompt distinguishes it from a term that was checked and
  // found wanting.
  //
  // S7 is the demonstration. "How does the lobula compare with the medulla?"
  // resolved the medulla, failed on the lobula, queried the medulla, and wrote:
  // "VFB does not currently hold data describing the specific anatomical
  // layers, developmental origins, or synaptic terminal counts for the lobula."
  // VFB holds all three for the lobula. The lobula was never looked at, and the
  // shelf — which is the block the model reads to find out what was looked at —
  // had nothing at all to say about it.
  //
  // The counts are deliberately absent here, because there are none: nothing
  // resolved, so nothing was catalogued. That is the whole point of the block.
  const named = (unmatched || []).filter(u => u?.name && !u.speculative)
  if (named.length) {
    parts.push('\nNAMES THAT DID NOT MATCH — these were looked for and no VFB term was found FOR THE WORDING USED.'
      + ' This is a fact about the lookup, NOT about VFB\'s holdings, and it is the weakest state on this list:'
      + ' nothing was catalogued for them, so nothing above describes them and nothing here licenses an absence about them.'
      + ' You are FORBIDDEN to write that VFB holds no data on any of these, or to contrast them with a term that DID resolve'
      + ' as though the difference were a difference in what VFB holds — it is a difference in what was found by name.'
      + ' Say instead that the name could not be matched, and name the closest records found.\n'
      + named.map(u => `  ${u.name}${u.candidates?.length ? ` — closest records found: ${u.candidates.slice(0, 3).join(', ')}` : ' — no near matches returned'}`).join('\n'))
  }
...
export function renderCoverageBlock(ledger, { floor = true } = {}) {
  const shelf = renderShelf(buildShelf(ledger), { unmatched: unmatchedNames(ledger) })
