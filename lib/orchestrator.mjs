...
import { isIndividualImageQuery, querySemantics, isGeneExpressionQuestion, isDriverLineQuestion, isAboutVfbItself, asksIntrinsic, INTRINSIC_RE } from './queryTypes.mjs'
...
async function synthesise(ledger, deps, models) {
  ...
  // CODE IS SUPPLIED. The user asked how to get this programmatically, and the run
  // knows the exact ids it resolved and the exact catalogue queries it ran, so a
  // correct runnable snippet is put into the answer deterministically after
  // synthesis (lib/reproduce.mjs). What remains is stopping the model writing a
  // second, worse one beside it.
  //
  // Two measured failures, both in production, and the second is why prose alone
  // is not enough. Asked "how would I get that same result in Python with
  // vfbquery?", it produced `vc.oc.get_instances`, `gen_short_form` and a
  // `vfb_type_2_skids` helper — recalled from a VFB_connect that predates
  // `vfb.term()`, and `vfb_type_2_skids` exists nowhere in that repository at
  // all. Displacing that snippet fixed the code and not the sentences: asked
  // again with the grounded block present, it spent four paragraphs arguing that
  ...
  //
  // The root cause is not a bad snippet, it is that recalled API knowledge reads
  // exactly like retrieved fact. The model has no library reference in front of
  // it, so any claim it makes about what a library can or cannot do is a guess
  // wearing the same clothes as a citation. The fix is to take the subject away
  // from it entirely, not to ask it to be more careful.
  //
  ...
  const codeBlock = (wantsReproduction(ledger.question) && hasCoordinates)
    ? '\n\nCODE IS ALREADY SUPPLIED FOR THIS ANSWER. This question asks how to get the result programmatically, and a correct, runnable snippet — built from the exact ids this conversation resolved and the exact queries it ran — is added to your answer automatically. So: do NOT write code, in any language. Do NOT walk through the code, number its steps, or name library functions, methods, attributes or arguments. Above all, do NOT say what a library can or cannot do, does or does not support, or whether something can be done "in one step" or "directly" — you do not have its API in front of you, your recollection of it is years out of date, and the snippet already does what was asked. Do not gesture at an unnamed one either — not "a VFB function", "the relevant method", "the appropriate command" — a sentence that names no mechanism and then points at one is worse than saying nothing about it. This overrides EVIDENCE: a documentation page in there may show a library method, and it is not the method for this question and not your subject — do not name it, do not explain what it is for, and do not contrast it with what was asked ("get_similar_neurons is designed for morphology, not for subclasses" is exactly the sentence not to write; the reader asked about subclasses and never mentioned the other thing). Write one or two sentences of plain prose saying what the result is, in terms of the data, and stop there.'
    : ''
...
export function dropUnofferedTemplateSteps(ledger, log = () => {}) {
  ...
  for (const step of ledger.plan || []) {
    if (!step.via_template || step.status === 'satisfied') continue
    ...
    log({ withdraw: 'vfb_run_query', query_type: step.args?.query_type, id: t.id, reason: 'template-query-not-offered' })
  }
