...
import { throwIfAborted, DEFAULT_RUN_DEADLINE_MS } from './runSignal.mjs'
 ...
 *
 * Half the default run deadline. The escalation costs up to three MCP rounds
 * plus a full synthesis, and the observed cost of the whole second pass on a
 * healthy run is 60-90 s, so half the deadline leaves the second pass room to
 * finish rather than to be killed halfway through and lose the first answer too.
 * Read from the deadline rather than written as a number so the two cannot drift
 * apart.
 */
export const ABSENCE_ESCALATION_DEADLINE_MS = Math.round(DEFAULT_RUN_DEADLINE_MS / 2)
