...
export async function maybeEscalateBeforeAbsence(ledger, answer, deps, log = () => {}) {
  ...
  // The QUERIES are VFB's own vocabulary and are the useful half. The claimed
  // sentence is prose the model wrote by restating the question, so it goes
  // through safeText and appears in full only under VFB_HARNESS_TRACE.
  console.error(`[VFBchat] ABSENCE ESCALATION | queries=${picks.map(p => `${p.query_type}@${p.id}`).join(',')}`
    + ` | claimed=${safeText(claims[0].text.trim(), { max: 120 })}`)
...
export function gateAbsence(ledger, answer, log = () => {}) {
  ...
  try {
    ...
    console.error(`[VFBchat] ABSENCE GATE | removed=${repairs.length} | licensed=false | ${safeText(repairs[0], { max: 160 })}`)
    return repaired
  } catch (e) {
