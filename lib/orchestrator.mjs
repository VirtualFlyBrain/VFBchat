...
async function resolveTerms(ledger, names, deps, models, log, speculative = new Set()) {
  ...
  await Promise.all(names.map(async (name) => {
    ...
    const directId = embedded.length === 1 ? embedded[0] : null
    if (directId && norm(directId) !== norm(name)) {
      log({ resolve_embedded_id: name, id: directId })
    }
    let firstSearch = directId ? null
      : await raceDeadline(deps.runTool('vfb_search_terms', { query: name, rows: 30, minimize_results: false }), budget.remaining())
