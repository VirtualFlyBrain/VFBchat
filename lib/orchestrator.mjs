...
import { serviceIdentityBlock } from './serviceIdentity.mjs'
...
import { decideTurnLanguage, isEnglish, languageName } from './language.mjs'
...
import { summariseClassPartners, isClassConnectivityPayload, isConnectivityToolPayload, partnerTable, partnerGraph } from './classPartners.mjs'
import { vfbQueryUrl } from './followOns.mjs'
import { summariseSplitStocks } from './splitStocks.mjs'
import { parseHierarchy, summariseHierarchy, renderHierarchyMarkdown } from './hierarchyTree.mjs'
...
/**
 * Tell the caller that the draft already streamed to the reader is being
 * abandoned, so it can take it off the screen before the replacement arrives.
 *
 * Synthesis streams token by token into a single live bubble on the client, and
 * the only thing that ever ends that bubble is the final `result` event. So when
 * a second synthesis runs — which is exactly what absence escalation buys — its
 * tokens are appended to the first draft's, and the reader watches the same
 * answer written out twice, in two slightly different wordings, until `result`
 * lands and replaces the pair. On the flagship "how many neurons" question that
 * window is a whole extra round of queries plus a second synthesis, and if the
 * connection drops inside it the duplicate is what the reader keeps.
 *
 * Discarding on the SERVER is not possible — the tokens are already sent. The
 * client is the only place that can unsee them, so it is told. Never throws:
 * a caller that has not wired this still gets the old behaviour rather than a
 * failed request.
 */
function discardDraft(deps, reason = '') {
  try { deps.onDraftDiscarded?.({ reason }) } catch { /* best-effort, like status */ }
}
...
export async function runHarness(question, deps) {
  ...
  // Which branch produced the plan. The language decision reads it: a clicked
  // chip inherits the conversation's language, every other branch is either a
  // typed question the planner read or an English-syntax match.
  let planVia = 'planner'
  if (plan) {
    planVia = 'focus'
  ...
  // A symbol the question names that the plan does not: shown the prior
  // turns' ids, the planner tends to leave terms_to_resolve empty (it already
  // "knows" the subject) or to read a new symbol as the previous turn's term.
  // Either way the question's own symbol goes up for resolution — the ledger
  // must know it, or the answer is labelled with the wrong class and opens
  // with "could not be matched" for a name VFB resolves in one search.
  {
    const named = unplannedQuestionSymbols(question, plan.terms_to_resolve || [], seedLedgerFromContext({ terms: {}, registry: {} }, priorContext))
    if (named.length) {
      plan.terms_to_resolve = [...(plan.terms_to_resolve || []), ...named]
      log({ step: 'adopt_question_symbols', names: named })
    }
  }
  ...
  // The language this turn is answered in, decided once and read everywhere
  // downstream (the resolver's translation rung, the silent synthesis stream,
  // and the translation pass in route.js). See lib/language.mjs for the rules.
  {
    const decided = decideTurnLanguage({
      planLanguage: plan.language,
      requestedLanguage: plan.requested_language,
      priorLang: priorContext.lang,
      via: planVia
    })
    ledger.language = decided.code
    ledger.langContext = decided.lang
    log({ step: 'language', code: decided.code, via: planVia, pinned: decided.lang.pinned })
    // "Can you reply in Persian?" with nothing else asked. There is no lookup to
    // run: the answer is the previous turn's answer, in the language asked for,
    // and route.js has the previous turn. Returned like a clarification — a
    // turn with no evidence — but flagged so it is rendered rather than asked.
    if (plan.language_request_only) {
      log({ step: 'language_request', code: decided.code })
      return { answer: '', ledger, trace, clarify: false, languageSwitch: true }
    }
  }
  ...
  while (guard++ < hardCap) {
    ...
    if (action.action === 'resolve_terms') {
      ...
      const questionSymbols = unplannedQuestionSymbols(question, action.terms, ledger).filter(n => !ledger.terms[n])
      const speculative = [
        ...intrinsicTermNames(question, action.terms),
        ...questionSymbols
      ].filter(n => !ledger.terms[n])
      ...
      maybeInjectSplitStocksStep(ledger, question, log)
      maybeInjectHierarchyStep(ledger, question, log)
...
export async function maybeEscalateBeforeAbsence(ledger, answer, deps, log = () => {}) {
  ...
  discardDraft(deps, 'absence-escalation')
 ...
 */
// A symbol the question names that the planner did not put up for resolution.
//
// In a conversation the planner is shown the prior turns and told to resolve
// back-references from them — and it over-applies that: asked "what are the
// main inputs to KCg-s?" after a turn about gamma dorsal Kenyon cell (KCg-d),
// it read KCg-s as a reference to the previous neuron and planned the
// question on KCg-d. The answer then ranked the wrong class's inputs under
// a "could not be matched" apology. A symbol in the question — a token with
// a digit or an internal hyphen and mixed case, the shape of a cell-type
// symbol, a driver name or an individual's label — is a name in its own
// right: if no planned term and no carried term contains it, it is put up
// for resolution alongside the planner's terms. One search at most; a symbol
// VFB does not know resolves to nothing and is reported as such.
const QUESTION_SYMBOL_RE = /\b(?=[A-Za-z0-9'_/-]*(?:\d|[a-z][A-Z]|[A-Z][a-z]+-[A-Za-z]))[A-Za-z][A-Za-z0-9'_/-]*(?:[-_][A-Za-z0-9'/]+)*\b/g
const QUESTION_SYMBOL_STOP = new Set(['vfb', 'split-gal4', 'split-gal4s', 'split-lexa', 'p65-ad', 'gal4-dbd', 'gal4-ad', 'lexa-dbd', 'ad-dbd', 'x-ray', 'e-mail', 'co-expression', 'id', 'ids', '3d', 'em', 'nblast', 'flywire', 'hemibrain', 'banc', 'malecns', 'fafb', 'manc', 'fanc', 'gal4', 'lexa', 'scrnaseq', 'rnaseq', 'rna', 'dna', 'top', 'uk', 'jrc2018u', 'jrc2018unisex', 'l1em', 'catmaid', 'neuprint', 'flybase', 'fbbt', 'obo', 'api', 'mcp', 'gpt', 'llm', 'ai', 'covid', 'ncbi', 'pubmed', 'doi', 'pmid', 'x', 'y', 'z', 'um', 'nm', 'µm', 'ii', 'iii', 'iv', 'h2o'])

export function unplannedQuestionSymbols(question = '', planned = [], ledger = {}) {
  const q = String(question || '')
  const out = []
  const covered = [
    ...planned.map(n => norm(n)),
    ...Object.values(ledger?.terms || {}).flatMap(t => [t?.label, t?.digest?.name, t?.digest?.symbol].filter(Boolean).map(norm)),
    ...Object.keys(ledger?.registry || {})
  ]
  for (const m of q.matchAll(QUESTION_SYMBOL_RE)) {
    const sym = m[0].replace(/^[-_']+|[-_'/]+$/g, '')
    const key = norm(sym)
    if (sym.length < 3 || QUESTION_SYMBOL_STOP.has(key)) continue
    if (/^(?:VFB|FBbt|FBgn|FBal|FBti|FBtp|FBco|FBst)[_:]/i.test(sym)) continue // an id resolves on its own path
    if (/^\d+(?:[.,]\d+)?[a-z]{0,2}$/i.test(sym)) continue // a number with a unit
    if (covered.some(c => c.includes(key)) || out.some(o => norm(o) === key)) continue
    out.push(sym)
  }
  return out
}

// The other half of unplannedQuestionSymbols. When a symbol the question
// names resolved to a VFB term, and the planner had substituted a term the
// question does NOT name (a back-reference read into a question that made
// none), the planner's term is displaced: removed from the ledger, and any
// planned step aimed at its id re-aimed at the symbol's. Only when the
// question carries no pronoun — "inputs to KCg-s, and does it overlap with
// that one" is legitimately about both.
const BACK_REFERENCE_RE = /\b(it|its|they|them|their|these|those|that one|the same|this one|the previous|the above|the former|the latter)\b/i

export function displaceBackReferences(ledger, question = '', symbols = [], log = () => {}) {
  const q = norm(question)
  if (!symbols.length || BACK_REFERENCE_RE.test(q)) return
  const symbolTerms = symbols.map(n => ledger.terms?.[n]).filter(t => t && t.id)
  if (!symbolTerms.length) return
  const symbolIds = new Set(symbolTerms.map(t => t.id))
  const mentioned = (t, name) => [name, t?.label, t?.digest?.name, t?.digest?.symbol]
    .filter(Boolean).map(norm).some(n => n && q.includes(n))
  for (const [name, t] of Object.entries(ledger.terms || {})) {
    if (!t?.id || symbols.includes(name) || symbolIds.has(t.id) || mentioned(t, name)) continue
    const replacement = symbolTerms[0]
    for (const step of (ledger.plan || [])) {
      if (step?.args?.id === t.id) {
        step.args = { ...step.args, id: replacement.id }
        step.note = `${step.note ? `${step.note}; ` : ''}re-aimed from ${t.id} (not named in the question) to ${replacement.id}`
      }
    }
    delete ledger.terms[name]
    log({ displace: name, id: t.id, by: replacement.id, reason: 'question-names-symbol' })
  }
}
...
export function intrinsicTermNames(question, names = []) {
  ...
  for (const name of names) {
    ...
    if (!base) continue
    if (INTRINSIC_RE.test(base)) {
      // A well-formed class name ("mushroom body intrinsic neuron") is left
      // alone. But the planner also copies the whole noun phrase — "neuron
      // types intrinsic to the mushroom body" — and that contains the word
      // without being a class, so skipping it left NO intrinsic term at all,
      // the ledger empty, and the literature gate open: the answer named the
      // MBONs, from a paper snippet, as the MB's intrinsic neurons (issue #44).
      // Take the region out of the phrase instead.
      if (/\bintrinsic\s+neurons?$/i.test(base)) continue
      const m = base.match(/\bintrinsic\s+(?:to|of|in|within)\s+(?:the\s+)?(.+?)\s*[?.!]?$/i)
      if (!m) continue
      base = m[1].trim()
      if (!base) continue
    }
...
/**
 * The standard English name for an anatomical, cell-type or gene name written
 * in another language — the wording VFB's index can answer to. Returns '' when
 * the model cannot say, or when the call fails; the caller treats both as "no
 * translation" and the name stays unmatched, exactly as before this rung.
 */
export async function englishNameFor(name, language, deps, models = {}) {
  const messages = [
    {
      role: 'system',
      content: `You translate names of Drosophila anatomical structures, neuron types, cell types, genes and other biological entities from ${languageName(language)} into the standard English name used by FlyBase and Virtual Fly Brain (e.g. "ellipsoid body", "mushroom body", "Kenyon cell", "antennal lobe projection neuron"). Keep gene symbols, GAL4 and split-GAL4 line names, dataset names (FlyWire, hemibrain, MANC, BANC, FAFB) and any identifiers exactly as written. Drop grammatical endings and articles. If the name is already English, return it unchanged. If you cannot translate it, return an empty string. JSON only.`
    },
    { role: 'user', content: JSON.stringify({ name: String(name), language: String(language) }) }
  ]
  try {
    const r = await deps.callStructured({ messages, schema: ENGLISH_NAME_SCHEMA, schemaName: 'english_term_name', model: models.extract })
    const out = r?.ok ? String(r.value?.english_name || '').trim() : ''
    // A "translation" that is a sentence is not a name.
    return out.length > 0 && out.length <= 80 && !/[.!?]\s|\n/.test(out) ? out : ''
  } catch {
    return ''
  }
}
...
async function resolveTerms(ledger, names, deps, models, log, speculative = new Set()) {
  ...
  await Promise.all(names.map(async (name) => {
    ...
    // SPELLING. Nothing above rewrites a misspelt word, so a name carrying one
    // has failed every rung and would be reported as unmatched. See
    // spellingMatchId for the acceptance rule; the probes only supply
    // documents, and the id has to earn its way through that guard. Only where
    // the name AND every variant of it returned nothing: a search that found
    // documents has candidates to show, and the extra round trips are spent
    // where an abstention with no candidates at all is the alternative.
    if (!directId && !resolvedId && searchIsEmpty(search)) {
      for (const probe of spellingProbes(name)) {
        if (budget.expired()) {
          ladderCutShort = true
          log({ resolve_budget_spent: name, at: 'spelling', ms: budget.spent(), skipped: probe })
          emit(deps, 'Lookup is slow — answering from what has resolved so far', 'tool')
          break
        }
        const probeRaw = await raceDeadline(deps.runTool('vfb_search_terms', { query: probe, rows: 30, minimize_results: false }), budget.remaining())
        if (probeRaw === TIMED_OUT) {
          ladderCutShort = true
          log({ resolve_budget_spent: name, at: 'spelling', ms: budget.spent(), skipped: probe })
          emit(deps, 'Lookup is slow — answering from what has resolved so far', 'tool')
          break
        }
        const probed = parseMaybe(probeRaw)
        if (searchIsEmpty(probed)) continue
        const hit = spellingMatchId(probed, name)
        if (hit) {
          search = probed
          resolvedId = hit.id
    ...
    // TRANSLATION. A name written in another language fails every rung above
    // for a reason none of them can fix: VFB's labels and synonyms are English
    // and the index does not translate, so "ellipszis test" returns nothing
    // (or, worse, junk) however it is singularised or respelt. The planner is
    // told to write English names on a non-English turn, which is what usually
    // makes this rung unnecessary; it is the backstop for the name the planner
    // copied as written. One structured call, one more search, and the id still
    // has to earn its way through the ladder — the translation only supplies a
    // wording. Symbols and line names are left as they are by the prompt.
    if (!directId && !resolvedId && !isEnglish(ledger.language) && typeof deps.callStructured === 'function' && !budget.expired()) {
      const english = await englishNameFor(name, ledger.language, deps, models)
      if (english && norm(english) !== norm(name)) {
        const trRaw = await raceDeadline(deps.runTool('vfb_search_terms', { query: english, rows: 30, minimize_results: false }), budget.remaining())
        if (trRaw === TIMED_OUT) {
          ladderCutShort = true
          log({ resolve_budget_spent: name, at: 'translation', ms: budget.spent(), skipped: english })
          emit(deps, 'Lookup is slow — answering from what has resolved so far', 'tool')
        } else {
          const translated = parseMaybe(trRaw)
          if (!searchIsEmpty(translated)) {
            const hit = exactTermMatchId(translated, english) || pickBestTermId(translated, english)
            if (hit) {
              search = translated
              resolvedId = hit
              matchedAs = english
              ...
              // documents where the original found none.
              search = translated
            }
          }
        }
    ...
    // KIND GUARD. "Find neurons similar to DA1 using NBLAST" resolved DA1 to
    // FBbt_00007473, larval dorsal tracheal anastomosis 1 — an exact synonym
    // match, which the ladder rightly puts above everything else — and NBLAST
    // was then run on a tracheal branch. The question said "neurons"; the
    // search knows which of its documents are neurons; nothing had joined the
    // two. When the question names a kind and the chosen document is not of
    // that kind, search again WITHIN the kind and let the ladder choose from
    // those. One extra search, only when the kinds disagree, and only a
    // neuron-shaped question is guarded — that is the one where a wrong kind
    // is silently fatal (the tools all accept the id and return nothing).
    let alternatives = []
    if (!directId && resolvedId && !budget.expired()) {
      const kind = questionKindHint(ledger.question)
      const chosen = validSearchDocs(search).find(d => sfOf(d) === resolvedId)
      // Only a SYMBOL matched on a SYNONYM is guarded. A name that is a term's
      // own label ("medulla", asked about its neurons) resolved correctly, and
      // the region rule downstream is what answers "neurons in the medulla";
      // re-searching it within neurons would replace the region with one of
      // its cell types. And a mock document with no facets says nothing about
      // its kind, so it is left alone too.
      const symbolLike = toks(name).length === 1 && /\d|^[A-Z]{2,}/.test(name.trim())
      const facetsKnown = Array.isArray(chosen?.facets_annotation) && chosen.facets_annotation.length > 0
      if (kind === 'neuron' && chosen && symbolLike && facetsKnown && !docIsNeuron(chosen) && norm(docLabel(chosen)) !== norm(name)) {
        const kindRaw = await raceDeadline(
          deps.runTool('vfb_search_terms', { query: name, rows: 30, minimize_results: false, filter_types: ['neuron'] }),
          budget.remaining()
        )
        const kindSearch = kindRaw === TIMED_OUT ? null : parseMaybe(kindRaw)
        const alt = kindSearch && !searchIsEmpty(kindSearch) ? pickBestTermId(kindSearch, name) : null
        if (alt) {
          log({ resolve_kind_switch: name, from: resolvedId, to: alt, kind })
          resolvedId = alt
          search = kindSearch
        }
      }
      // CLASS LIFT. "EPG neurons" — a plural, a TYPE of neuron — resolved to
      // EPG-5L#3 (FAFB:4087066), one reconstructed cell, because "EPG neuron"
      // matches that cell's synonym and the class carries the symbol only as
      // "EPG". The answer then described one neuron's 38 partners as what VFB
      ...
      if (liftable) {
        ...
        if (symbol && !budget.expired()) {
          const classRaw = await raceDeadline(
            deps.runTool('vfb_search_terms', { query: symbol, rows: 30, minimize_results: false, filter_types: ['class'] }),
            budget.remaining()
          )
          const classSearch = classRaw === TIMED_OUT ? null : parseMaybe(classRaw)
          const classId = classSearch && !searchIsEmpty(classSearch) ? exactTermMatchId(classSearch, symbol) : null
          if (classId) {
      ...
      // which reading, and name the others, or a user who meant the larval motor
      // neuron gets a projection-neuron answer with no hint that a choice was made.
      alternatives = sameKindAlternatives(search, name, resolvedId)
    }
    ...
    if (!id) {
      ...
      const datasetNames = datasetMatches && datasetMatches.length <= DATASET_CANDIDATE_CAP
        ...
        : []
      let candidates = category && category.matches.length
        ? category.matches.map(d => docLabel(d)).filter(Boolean).slice(0, 6)
        : (datasetNames.length ? datasetNames : searchCandidateLabels(search))
      // NOTHING AT ALL is the outcome the reader can do least with. "The term
      // 'visual processing' could not be matched" ends the conversation: it
      // names no term VFB does hold, so it gives the user nothing to ask next.
      // Before conceding, search the phrase's content words one at a time and
      // offer what THOSE find — "visual" reaches the visual system, the visual
      // projection neurons and the lamina; "processing" reaches nothing and is
      // dropped. These are marked LOOSE so the prompt presents them as terms
      // to rephrase around, never as readings of the phrase.
      let loose = false
      if (!candidates.length && !speculative.has(name) && !ladderCutShort && !budget.expired()) {
        candidates = await looseWordCandidates(name, deps, budget)
        loose = candidates.length > 0
        if (loose) log({ resolve_loose_candidates: name, candidates })
      }
      ...
      // simply nothing.
      addTerm(ledger, name, { id: null, attempted: true, candidates, loose, truncated: ladderCutShort, speculative: speculative.has(name) })
      log({ resolve: name, id: null, candidates: candidates.length, loose, truncated: ladderCutShort, speculative: speculative.has(name) })
      return
    }
    ...
    addTerm(ledger, name, { id: effectiveId, label: chosenLabel || undefined, publications, info, digest, fetchedId, superseded, attempted: true, alternatives,
...
export function backfillDigestPreview(ledger, args = {}, parsed = {}) {
  ...
  // Every row is VFB's own label→id pair, so every row is linkable — and until
  // now only five of them were. The term-info preview registers at most five
  // entities (VFB previews five rows; the digest caps `exampleEntities` at five
  // for prompt budget), and this function wrote rows into the digest without
  // registering any. "What are the subtypes of the gamma Kenyon cell?" listed
  // all ten subclasses and linked the first five (issue #62). The registry is
  // not prompt budget — first write wins, and the synthesiser never sees it —
  // so register every parsed row, whether or not the preview was already full.
  for (const r of parsedRows) recordTermId(ledger, r.name, r.id)
...
async function runStep(ledger, step, deps, paramsByName, models, log) {
  ...
  // An id nothing in this run supplied is an id the planner made up. "What
  // neurons are involved in visual processing?" searched the phrase (nothing),
  // then fetched FBbt_00007499 — an obsolete fan-shaped body segment — and the
  // answer explained that the fan-shaped body is involved in visual
  // integration. The id was in no search result, no resolved term, no prior
  // turn and not the question; the model recalled a shape and filled it in.
  // The ledger knows every id the run has actually seen, so the check is
  // cheap and exact: withdraw the step, and let the search recovery below put
  // the phrase through the resolver instead.
  if (args?.id && /^(FBbt|VFB|FBgn|FBal|FBti|FBtp|FBco)_/i.test(String(args.id)) && !idIsSourced(ledger, deps, String(args.id))) {
    withdrawStep(ledger, step.id, `withdrawn: ${args.id} was not returned by any search or lookup in this conversation`)
    log({ withdraw: step.tool, step: step.id, id: args.id, reason: 'unsourced-id' })
    return
  }
  ...
  // Deterministic hierarchy answer: the claim counts and names the direct
  // children; the whole tree is rendered as a nested list after the prose.
  if ((step.tool === 'vfb_get_hierarchy' || parsed?.tool === 'vfb_get_hierarchy') && parsed && typeof parsed === 'object' && !parsed.error) {
    const tree = parseHierarchy(parsed)
    const hs = summariseHierarchy(tree)
    if (hs) {
      if (tree.id) recordTermId(ledger, tree.label, tree.id)
      for (const r of hs.rows) recordTermId(ledger, r.name, r.id)
      addEvidence(ledger, buildEvidenceRow({
        source: 'vfb',
        claim: hs.claim,
        verbatim: JSON.stringify({ tool: step.tool, id: tree.id, relationship: tree.relationship, direct: hs.directCount, total: hs.totalCount }),
        locator: { stepId: step.id, tool: step.tool }
      }))
      const displayKey = `hierarchy:${tree.id}:${tree.relationship}`
      ledger.displayKeys ||= new Set()
      if (!ledger.displayKeys.has(displayKey)) {
        ledger.displayKeys.add(displayKey)
        ;(ledger.appendices ||= []).push(renderHierarchyMarkdown(tree))
      }
      log({ run_step: step.id, tool: step.tool, answered: true, via: 'hierarchy', direct: hs.directCount, total: hs.totalCount })
      return
    }
  }

  // Deterministic split-stocks answer. The macro's payload is already the
  // answer; the claim and the table are written from it here so the numbers
  // (stock centre, stock number) never pass through a model.
  if ((step.tool === 'vfb_find_split_stocks' || parsed?.tool === 'vfb_find_split_stocks') && parsed && typeof parsed === 'object' && !parsed.error) {
    const ss = summariseSplitStocks(parsed)
    if (ss) {
      if (parsed.resolved?.id) recordTermId(ledger, parsed.resolved.label, parsed.resolved.id)
      for (const r of ss.rows) recordTermId(ledger, r.name, r.id)
      addEvidence(ledger, buildEvidenceRow({
        source: 'vfb',
        claim: ss.claim,
        verbatim: JSON.stringify({ tool: step.tool, resolved: parsed.resolved, split_count: parsed.split_count, splits: parsed.splits }),
        locator: { stepId: step.id, tool: step.tool }
      }))
      const displayKey = `split-stocks:${parsed.resolved?.id || ''}`
      ledger.displayKeys ||= new Set()
      if (!ledger.displayKeys.has(displayKey)) {
        ledger.displayKeys.add(displayKey)
        ;(ledger.tables ||= []).push(ss.table)
      }
      log({ run_step: step.id, tool: step.tool, answered: true, via: 'split_stocks', splits: parsed.splits?.length || 0 })
      return
    }
    if (parsed.note) {
      addEvidence(ledger, buildEvidenceRow({
        ...
        verbatim: JSON.stringify({ tool: step.tool, resolved: parsed.resolved, split_count: parsed.split_count }),
        locator: { stepId: step.id, tool: step.tool }
      }))
      log({ run_step: step.id, tool: step.tool, answered: true, via: 'split_stocks', splits: 0 })
      return
    }
  }
  ...
  //
  // …and keyed on BOTH payload shapes. The class-level partner TOOL
  // (vfb_find_connectivity_partners) returns the same table pre-ranked by total
  // weight and truncated, and it is what the injector and the planner reach for
  // on a class — so on the most common route the ranker was never reached, the
  // raw JSON went to the extractor, and the prose led with "adult neuron" and
  // "Kenyon cell" (issue #47). Both shapes now land here.
  if ((step.tool === 'vfb_run_query' && isClassConnectivityPayload(parsed)) || isConnectivityToolPayload(parsed)) {
    const queryId = String(args.id || parsed?.endpoint?.id || '')
    const term = Object.values(ledger.terms).find(t => t.id === queryId)
    ...
    if (s) {
      ...
      // The table is the display; the graph is the optional follow-up
      // (issue #50). Both are built from the RANKED partners, so what the reader
      // sees is what the claim says, and neither depends on the synthesiser.
      const qt = String(args.query_type || parsed?.endpoint?.query_type ||
        (s.direction === 'upstream' ? 'UpstreamClassConnectivity' : 'DownstreamClassConnectivity'))
      // Once per class and direction: a step can be run again within a turn.
      // And only for a CONNECTIVITY question: the absence check probes these
      // queries on a 25-row page before saying VFB has no data, and a "top 3"
      // partner table drawn from that page under "list the DA1 lPN neurons"
      // is neither asked for nor true.
      const displayKey = `${s.queryId || s.queryLabel}:${s.direction}`
      ledger.displayKeys ||= new Set()
      if (hasConnectivityIntent(ledger.question || '') && !ledger.displayKeys.has(displayKey)) {
        ledger.displayKeys.add(displayKey)
        const table = partnerTable(s, { queryUrl: s.queryId && qt ? vfbQueryUrl(s.queryId, qt) : '' })
        if (table) (ledger.tables ||= []).push(table)
        const graph = partnerGraph(s)
        if (graph) (ledger.graphs ||= []).push(graph)
      }
      addEvidence(ledger, buildEvidenceRow({
        source: 'vfb',
        claim: s.claim,
  ...
  // A planner search that found nothing is not the end of the name. The
  // planner writes the user's phrase into vfb_search_terms verbatim, often with
  // a type filter, and VFB's index answers "visual processing" with zero
  // documents — while the resolver, which was never consulted because the
  // phrase was never in terms_to_resolve, would have walked its ladder to
  // "visual system" in one more search. Hand the phrase to the resolver and
  // withdraw the step: a search that found nothing is not evidence of absence,
  // and left as not_found it would be written up as one.
  const searchQuery = step.tool === 'vfb_search_terms' ? String(args?.query || '').trim() : ''
  if (searchQuery && searchIsEmpty(parsed) && !ledger.terms[searchQuery] && !(ledger.termsToResolve || []).includes(searchQuery)) {
    ledger.termsToResolve = [...(ledger.termsToResolve || []), searchQuery]
    withdrawStep(ledger, step.id, `withdrawn: the search for "${searchQuery}" returned nothing; the name goes through the resolver instead`)
    log({ withdraw: step.tool, step: step.id, query: searchQuery, reason: 'empty-search-to-resolver' })
    return
  }
...
async function retrieve(ledger, plan, deps, models, log) {
  if (plan.documentation) {
    ...
    // still one fetch.
    // …and read the pages whose title or URL shares a topic word with the
    // question before the ones that merely rank: the extractor is generous
    // about "answered", so for "How many larval dataset?" the neuron-counts
    // page, ranked first on the word "many", answered with a count of
    // neurons, and the page that groups datasets by stage was never opened.
    const candidates = pickDocCandidates(search, DOC_CANDIDATES)
      .map((c, i) => ({ c, i, rel: docCandidateRelevant(ledger.question, c) ? 0 : 1 }))
      .sort((a, b) => a.rel - b.rel || a.i - b.i)
      .map(x => x.c)
  ...
  if (plan.literature) {
    // A literature lookup that falls over must not take the answer with it.
    //
    // This step is three calls — a PubMed search, an efetch or bioRxiv fetch,
    // and an ELM structured extract over up to MAX_EXTRACT_CHARS of article
    // text. Any of them can time out, and until now a throw here propagated all
    // the way out of the run: the stream closed with no answer and no error,
    // because route.js classified the resulting AbortError as the user
    // abandoning the run. Reproduced against production on 11 August 2026 with
    // a two-turn question that ended "that VFB is on?" — the stream died after
    // "Checking the literature" at 103s, 110s and 160s across three attempts.
    //
    // Supporting evidence is a SUPPLEMENT to a VFB answer, never the answer, so
    // failing to get it is a reason to say the lookup did not complete — the
    // TRIED, NO RESULT state the coverage model already defines — not a reason
    // to return nothing at all. A genuine run abort still propagates: the whole
    // point of the fixed predicate is that those two are now distinguishable.
    try {
      await retrieveLiterature(ledger, deps, models, log)
    } catch (err) {
      if (isRunAbortedWith(err, deps?.signal)) throw err
      log({ retrieve: 'literature', failed: String(err?.message || err) })
    }
  }
}

/** The literature branch, extracted so its failure can be contained. */
async function retrieveLiterature(ledger, deps, models, log) {
  {
...
// "many" and "much" are question words, not topic words: "How many larval
// datasets?" shared "many" with the neuron-counts page ("How many neurons…")
// and nothing with the page that groups datasets by stage, whose title says
// "datasets" where the question said "dataset" — so the wrong page was read.
// Tokens are stemmed to their singular for the same reason.
const DOC_REL_STOP = new Set(['the', 'and', 'are', 'for', 'how', 'what', 'when', 'where', 'which', 'who', 'does', 'vfb', 'virtual', 'fly', 'brain', 'data', 'many', 'much', 'there', 'have', 'hold', 'holds'])
const docStem = (w) => (w.length > 4 && w.endsWith('ies') ? `${w.slice(0, -3)}y` : w.length > 3 && w.endsWith('s') && !w.endsWith('ss') ? w.slice(0, -1) : w)
function docTokens(s = '') {
  return new Set(String(s).toLowerCase().match(/[a-z0-9]+/g)?.map(docStem).filter(w => w.length > 2 && !DOC_REL_STOP.has(w)) || [])
}
...
export function docCandidateRelevant(question, result) {
...
async function ingestParallelDocs(ledger, docSearchPromise, deps, models, log) {
  ...
  const top = pickDocCandidates(search?.results ? search : search?.response, DOC_CANDIDATES)
    .find(c => docCandidateRelevant(ledger.question, c))
  if (!top) return
  try {
...
async function synthesise(ledger, deps, models) {
  // VFB rows first, then documentation, then literature — a stable partition,
  // not a shuffle. The prompt's rule that a database fact outranks a paper's
  // claim only governs labelling; what the model reads FIRST is what it leads
  // with, and a literature snippet inserted ahead of the ontology row is how
  // the MBONs were named as the mushroom body's intrinsic neurons (issue #44).
  const sourceRank = (src) => (src === 'vfb' ? 0 : /doc/i.test(String(src || '')) ? 1 : 2)
  const evidence = ledger.evidence
    .map((e, i) => ({ e, i }))
    .sort((a, b) => sourceRank(a.e.source) - sourceRank(b.e.source) || a.i - b.i)
    .map(({ e }) => ({
  ...
  const unmatched = unmatchedTermEntries(ledger)
    .map(t => {
      ...
      // LOOSE candidates share a word with the name, no more: they are not
      // readings of it. The reader's next move is a different question, so
      // give them one — phrased around a term VFB does hold.
      if (t.loose && cands.length) {
        return `"${name}" — VFB has no term by this name; that is a fact about VFB's naming, not its data. Terms VFB DOES hold that share a word with it: ${cands.join('; ')}. Do not present these as what the name means. Instead, end with two or three concrete rephrasings the user could ask next, each built around one of these terms (for example "which neurons have a part in the <term>?", "what are the subtypes of <term>?", "what does <term> connect to?").`
      }
      return cands.length
  ...
  // A name the resolver READ rather than matched — a symbol several terms
  // carry — is disclosed as a reading, with the others named, so the reader can
  // redirect in one message if the wrong one was taken.
  const readings = Object.values(ledger.terms || {})
    .filter(t => t.id && Array.isArray(t.alternatives) && t.alternatives.length && (t.digest?.name || t.label))
    .map(t => `"${t.label || t.digest?.name}" was read as ${t.digest?.name || t.label}. VFB also holds these under that name: ${t.alternatives.join('; ')}. Say in one clause which reading the answer uses, and name the others as alternatives the user can ask about.`)
  ...
  // reads English, and the translation step is what the reader sees. Nothing
  // had said so to the model, and on a Japanese question it wrote the draft in
  // Japanese about one run in three — the linker then found no "mushroom body"
  // to link, and the reader got an answer with no links (battery L3). The rule
  // is stated on the turns it applies to, and stated last so it is not buried.
  const englishDraftRule = isEnglish(ledger.language)
    ? ''
    : `\n\nWRITE THE ANSWER IN ENGLISH, even though the question is written in ${languageName(ledger.language)}. It is translated for the reader afterwards; an answer written in another language here cannot be linked or checked. Use the English names from RESOLVED ENTITIES exactly as written.`
  // The service's own version and model, appended to the SYNTHESIS system
  // message specifically.
  //
  // v4.2.4 shipped this block on route.js's `systemPrompt` — which is not the
  // prompt that writes the answer. The harness synthesises here, with the
  // message built below, so the facts never reached the model and it went on
  // deflecting "what version are you?" exactly as before. Verified in
  // production before this was moved: the release was live and the behaviour
  // unchanged.
  //
  // It goes last, after the evidence rules, so it cannot be read as licence to
  // treat these as evidence: they are facts about the SERVICE, not about the
  // data, and the block says so.
  const messages = [
...
NEVER OVERCLAIM — this is critical. VFB holds only PARTIAL data; what it has, or lacks, is not definitive of biology, and it is for the USER to interpret. Your job is to point the user at the relevant VFB data, not to draw conclusions for them. Therefore: (1) ABSENCE means only that VFB does not currently hold/annotate that data — it never means the thing does not exist, is not true, or is not connected. Never write "there are no X", "X does not connect to Y", or "X has no Y"; write "VFB does not currently hold data on …". (2) COUNTS and records are what VFB has ANNOTATED, not biological totals or complete sets — write "VFB holds N records" or "VFB has annotated N", never "there are N" / "X has N". (3) DATA-DERIVED facts (neurotransmitter, connectivity, classification, similarity) are evidence VFB records, often predicted or from one dataset — attribute them ("the connectome data indicates", "VFB records show", "predicted as") rather than asserting them as settled fact. (4) Do NOT speculate beyond the data or state functional, causal, or interpretive conclusions of your own. (5) NEVER attribute anything to "the literature", "publications", "published estimates", "papers report/claim", or similar UNLESS the EVIDENCE contains an actual citation for it (a PMID, DOI, or FlyBase reference) — and then cite that specific reference. If you do not have such a reference in EVIDENCE, do NOT make the literature/published claim at all and do NOT invent a number or a source; state only what the VFB data shows. Be constructive, but scoped: say what VFB DOES have where it bears on the question, and name an un-run query ONLY where AVAILABLE VFB DATA marks it WORTH SAYING. Never recite the rest of that block back to the reader — every query in it is already shown beside your answer as a clickable link, so listing them in prose pads the answer without adding anything. What you do say about VFB's holdings is a pointer to data for the user to judge, never your own determination. (6) ABSENCE REQUIRES A LOOKUP THAT HAPPENED. AVAILABLE VFB DATA sorts every query for the resolved terms into four states, and which sentence you are allowed to write is decided by the state, not by how the question was worded or how complete you want the answer to feel. RUN, WITH RESULTS — the results are in EVIDENCE; answer from them, and never say one of these has not been run. RUN, CAME BACK EMPTY — this state, and only this state, licenses an absence: write "VFB does not currently hold …" plainly, with no hedging and no suggestion that the query still needs running. TRIED, NO RESULT — the lookup was attempted and did not complete; that is NOT an empty result and NOT an absence, so say the lookup did not complete, or leave it out. HELD, NOT YET RUN — VFB holds these records and nothing queried them: you are FORBIDDEN to write "VFB does not currently hold data on …", "no data is available for …", or any other absence about them, however the question was worded, INCLUDING when the question asks for exactly this and you have nothing else to say about it; for the ones marked WORTH SAYING, state the HOLDING and its count ("VFB holds 92 transgene expression reports for Kenyon cell") and stop there. Never write that a query "has not been run yet", "still needs to be run", "was not executed", or anything else about this program's queue — the reader has no session and cannot run anything, so that sentence describes this process rather than VFB, and the follow-up query is already offered beside your answer as a clickable link. A holding is a SUPPLEMENT to an answer, never the answer: a reply consisting of a count and a note about a pending query is not an answer to anything. An empty EVIDENCE block means nothing was asked, and something nobody asked about is not something VFB does not have — if nothing ran and nothing relevant is listed at all, say this particular question has not been looked up, NOT that the data does not exist. (7) ANSWER THE QUESTION; DO NOT HAND BACK CODE. Unless the user asked how to do something programmatically, never answer with a vfb_connect, vfbquery, Python or shell snippet in place of the result — writing \`vfb.terms(['DA1 lPN'])\` and then saying the data is unavailable gives the reader a chore instead of an answer. Report what the queries returned.` + serviceIdentityBlock() },
  ...
  //
  // `silent`: the answer is written in English whatever the question's language
  // — every gate and linker downstream reads English — and translated at the end
  // (route.js). On a non-English turn the English draft must not be shown, so
  // the sink is asked to accumulate without emitting; the translation is what
  // streams to the reader.
  const silent = !isEnglish(ledger.language)
  if (typeof deps.callTextStream === 'function') {
    return await deps.callTextStream({ messages, model: models.synth, sourceQuotes, silent })
  }
  return await deps.callText({ messages, model: models.synth, sourceQuotes, silent })
}
...
export function maybeInjectConnectivityStep(ledger, question, log = () => {}) {
  ...
  // A CLASS that offers its own Upstream/DownstreamClassConnectivity query gets
  // that query, whole (limit 0), rather than the partner tool: the tool sorts
  // by total weight and keeps twelve rows, so the ranker downstream only ever
  // saw the ontology's top — the broad superclasses — and the specific partners
  // a fly neuroscientist would name were cut before ranking (issue #47). The
  // class table for a broad class runs to ten thousand rows; the cap keeps the
  // strongest 2,500 by VFB's own order, which is every row that could rank.
  const direction = connectivityDirection(question) || 'downstream'
  const classQuery = classConnectivityQuery(t.digest, direction)
  const cxStep = planned.find(s => s.status !== 'satisfied' && s.tool === 'vfb_find_connectivity_partners')
  if (classQuery?.query_type) {
    const cxArgs = { id: t.id, query_type: classQuery.query_type, limit: CLASS_CONNECTIVITY_ROW_CAP }
    if (cxStep) {
      cxStep.tool = 'vfb_run_query'
      cxStep.args = cxArgs
      cxStep.status = 'pending'
      cxStep.note = `retargeted to the class's own ${classQuery.query_type} (whole table, ranked deterministically)`
      log({ retarget: 'vfb_run_query', query_type: classQuery.query_type, id: t.id, reason: 'class-connectivity-table' })
      return
    }
    if (planned.length) return
    ledger.plan.push({
      ...
      args: cxArgs,
      status: 'pending',
      note: `auto-injected connectivity step (neuron class → run ${classQuery.query_type})`
    })
    log({ inject: 'vfb_run_query', query_type: classQuery.query_type, id: t.id, reason: 'class-connectivity-table' })
    return
  }
  ...
  ledger.plan.push({
    ...
    args: { endpoint_type: endpoint, direction },
    status: 'pending',
...
const CLASS_CONNECTIVITY_ROW_CAP = 2500

/** The class-level connectivity query a term's catalogue offers for a direction. */
function classConnectivityQuery(digest, direction = 'downstream') {
  const wanted = direction === 'upstream' ? /^UpstreamClassConnectivity$/i : /^DownstreamClassConnectivity$/i
  return (digest?.queries || []).find(q => wanted.test(q.query_type || ''))
}
...
export function maybeInjectScrnaseqStep(ledger, question, log = () => {}) {
  ...
  let terms = Object.values(ledger.terms || {}).filter(t => t.id && hasScrnaseqTerm(t))
  // A region has no clusters of its own; its cell types do. The tool makes
  // that hop (scrnaseqCellTypesInRegion), so a gene question about a single
  // resolved region is still routed to it rather than left to the absence rule.
  if (!terms.length) terms = Object.values(ledger.terms || {}).filter(t => t.id && kindOf(t) === 'region')
...
// "Show me the hierarchy of Kenyon cell subtypes" — a TREE question. The
// SubclassesOf query lists direct children as a flat table, and until now that
// flat table (or a sentence naming a dozen of its rows) was the whole answer,
// with "VFB does not provide a multi-level tree in this view" appended (issue
// #48). VFB does: get_hierarchy returns the nested tree, and the harness
// renders it. Fires on hierarchy/subtype/classification wording for a class
// that has subclasses, and on parts/subdivision wording for a region.
const HIERARCHY_INTENT_RE = /\b(hierarch(?:y|ies|ical)|taxonom(?:y|ic)|classif(?:y|ied|ication)|tree|sub-?types?|sub-?classes|(?:what|which)\s+(?:kinds?|sorts?|types?)\s+of|types?\s+of|sub-?divisions?|(?:what|which|list)(?:\s+\w+){0,2}\s+parts?\s+(?:of|make up|compose))\b/i
const HIERARCHY_DEPTH_WORDS_RE = /\b(full|whole|entire|complete|all\s+levels|every\s+level|nested|multi-?level)\b/i
const PARTS_WORDING_RE = /\b(parts?|sub-?divisions?|components?|compartments?|sub-?regions?)\b/i

export function maybeInjectHierarchyStep(ledger, question, log = () => {}) {
  const q = String(question || '')
  if (!HIERARCHY_INTENT_RE.test(q)) return
  // "the strongest downstream partner cell TYPES of EPG" asks for partners,
  // ranked; "types of" there is the unit of the answer, not a request for
  // EPG's own subtypes — which were being drawn as a tree under the ranking.
  // A connectivity question only gets the tree when it asks for one by name.
  if (hasConnectivityIntent(q) && !/\b(hierarch\w*|taxonom\w*|tree|sub-?types?|sub-?classes)\b/i.test(q)) return
  if (ledger.plan.some(s => /get_hierarchy/i.test(s.tool) && s.status !== 'withdrawn')) return
  const terms = Object.values(ledger.terms || {}).filter(t => t.id && /^FBbt_/.test(t.id) && !isIndividualTerm(t))
  if (terms.length !== 1) return
  const t = terms[0]
  const flags = termFlags(t)
  const queries = t.digest?.queries || []
  const offers = (re) => queries.some(qq => re.test(qq.query_type || ''))
  const isNeuron = flags.includes('neuron')
  const wantsParts = PARTS_WORDING_RE.test(q) && !isNeuron
  let relationship = ''
  if (wantsParts && (offers(/^PartsOf$/i) || flags.includes('synaptic_neuropil') || flags.includes('nervous_system'))) relationship = 'part_of'
  else if (offers(/^SubclassesOf$/i) || flags.includes('has_subclass')) relationship = 'subclass_of'
  if (!relationship) return
  const maxDepth = HIERARCHY_DEPTH_WORDS_RE.test(q) ? 3 : 2
  ledger.plan.push({
    id: `hy${ledger.plan.length + 1}`,
    tool: 'vfb_get_hierarchy',
    answers: [String(question)],
    args: { id: t.id, relationship, direction: 'descendants', max_depth: maxDepth },
    status: 'pending',
    note: `auto-injected hierarchy step (${relationship} tree of a ${isNeuron ? 'class' : 'region'})`
  })
  log({ inject: 'vfb_get_hierarchy', id: t.id, relationship, max_depth: maxDepth })
}

// "Which stocks can I order for the splits that target X" — a neuron CLASS
// asked a stocks question. The stocks card told the planner to use
// vfb_find_stocks, which takes a FlyBase FEATURE id, so on an anatomy term it
// had nothing to call, ran the expression query instead and told the user VFB
// does not list stock numbers (issue #46). It does: SplitsTargeting → the two
// hemidriver constructs → FindStocks on each → the stock carrying both. That
// chain is the vfb_find_split_stocks macro, injected here whenever the question
// is about stocks and exactly one resolved term is a neuron class.
const STOCKS_INTENT_RE = /\b(stocks?|fly\s+lines?|bloomington|bdsc|kyoto|vdrc)\b/i

export function maybeInjectSplitStocksStep(ledger, question, log = () => {}) {
  if (!STOCKS_INTENT_RE.test(String(question || ''))) return
  if (ledger.plan.some(s => /find_split_stocks/i.test(s.tool))) return
  const terms = Object.values(ledger.terms || {}).filter(t => t.id && /^FBbt_/.test(t.id) && isNeuronTypeTerm(t) && !isIndividualTerm(t))
  if (terms.length !== 1) return
  const t = terms[0]
  const name = t.digest?.name || t.label
  if (!name) return
  // Only a class whose catalogue offers SplitsTargeting can have split stocks;
  // on any other class the step would run to an honest "no splits", which the
  // digest already says for free.
  const queries = t.digest?.queries || []
  if (queries.length && !queries.some(q => /^SplitsTargeting$/i.test(q.query_type || ''))) return
  // The planner's own feature-id stock step cannot run on an anatomy term.
  for (const s of ledger.plan) {
    if (s.tool === 'vfb_find_stocks' && s.status === 'pending' && !/^FB(?:gn|al|ti|tp|co|st)/.test(String(s.args?.feature_id || ''))) {
      s.status = 'withdrawn'
      s.note = 'withdrawn: vfb_find_stocks takes a FlyBase feature id; replaced by vfb_find_split_stocks on the neuron class'
    }
  }
  ledger.plan = ledger.plan.filter(s => s.status !== 'withdrawn')
  ledger.plan.push({
    id: `st${ledger.plan.length + 1}`,
    tool: 'vfb_find_split_stocks',
    answers: [String(question)],
    args: { neuron_type: t.id },
    status: 'pending',
    note: 'auto-injected split-stocks step (stocks question, neuron class term)'
  })
  log({ inject: 'vfb_find_split_stocks', id: t.id })
}
...
/**
 * A resolved region that is the ontology CLASS rather than a registered image of
 * one. Painted domains carry IsPaintedDomain/IsIndividual; the class carries
 * IsClass. Older records that carry neither fall back to the SuperTypes flag.
 */
function isOntologyClassRegion(t) {
  if (t?.info?.IsIndividual === true || t?.info?.IsPaintedDomain === true) return false
  if (t?.info?.IsClass === true) return true
  return termFlags(t).includes('class')
}

/**
 * The one region a count question is about, or null when that cannot be decided.
 *
 * "Exactly one region resolved" was the rule, and it declines whenever a second
 * one does. That is not a rare accident. VFB's search answers "fly brain" with
 * 224 documents, and essentially all of them are painted domains on
 * JRC_FlyEM_Hemibrain — AB(L), AL(R), AL-DA1(R) and the rest. Every one of those
 * is flagged Anatomy, is not a neuron and is not a cell, so every one satisfies
 * isRegionTerm exactly as `brain` (FBbt_00005095) does. One extra survivor from
 * that set is enough to take the count off the table.
 *
 * A painted domain is a region drawn on one specimen. The curated counts are
 * about the region itself — the class — so prefer classes. Two classes is a
 * genuine ambiguity ("how many neurons in the mushroom body and the medulla?")
 * and still declines, because picking one silently answers half the question.
 */
function pickCountRegion(regionTerms) {
  if (regionTerms.length <= 1) return regionTerms[0] || null
  const classes = regionTerms.filter(isOntologyClassRegion)
  return classes.length === 1 ? classes[0] : null
}

/**
 * Append a region-neuron-count step for "how many neurons in <region>" questions
 * with a resolved region. This brings the curated published estimate (with its
 * citation) into evidence, so the answer can give a cited biological figure rather
 * than restating VFB's annotated NeuronsPartHere count as the total. Idempotent.
 *
 * WHY THIS DECIDES OUT LOUD.
 *
 * Every gate below used to be silent and only the success path called log().
 * log() appends to the per-request trace, and the trace is printed only when
 * VFB_HARNESS_TRACE=true — so on a production container the absence of an
 * `inject` line is not evidence that the injector declined. It is not evidence
 * either way. Three production runs were read that way in the 11 August defect
 * review and the conclusion drawn from them could not have been supported.
 *
 * So the decision now goes where the grounding audit, the absence gate and the
 * term-info failure report already go: stderr, one line, on both paths, and only
 * once the question has count intent — so it stays quiet on the questions this
 * has nothing to say about. Ontology ids and gate names only. The question is
 * the user's text and is deliberately not logged.
 */
export function maybeInjectRegionNeuronCountStep(ledger, question, log = () => {}) {
  ...
  const decide = (outcome, detail = '') => {
    log({ inject: 'vfb_get_region_neuron_count', outcome, detail: detail || undefined })
    console.error(`[VFBchat] REGION COUNT INJECTOR | ${outcome}${detail ? ` | ${detail}` : ''}`)
  }
  if (ledger.plan.some(s => /region_neuron_count/i.test(s.tool))) return decide('already-planned')
  const resolved = Object.values(ledger.terms || {}).filter(t => t.id)
  const regionTerms = resolved.filter(isRegionTerm)
  if (!regionTerms.length) {
    // The flags are the point of this line: isRegionTerm turns on them, and
    // "resolved three terms, none of them a region" is a different defect from
    // "resolved nothing".
    const flags = resolved.map(t => `${t.id}[${termFlags(t).join('+') || 'no-flags'}]`).join(',')
    return decide('no-region-term', `resolved=${resolved.length}${flags ? ` | ${flags}` : ''}`)
  }
  const t = pickCountRegion(regionTerms)
  if (!t) return decide('ambiguous-region', `regions=${regionTerms.map(r => r.id).join(',')}`)
  const name = t.digest?.name || t.label
  if (!name) return decide('region-has-no-label', `region_id=${t.id}`)
  ledger.plan.push({
  ...
  console.error(`[VFBchat] REGION COUNT INJECTOR | injected | region_id=${t.id}`)
}
...
const QUERY_INTENT_RULES = [
  ...
  // "show me" is not an image cue — it is how people ask for anything ("show
  // me the hierarchy of Kenyon cell subtypes" ran ListAllAvailableImages,
  // 32,328 rows, issue #48). Only "show me <images/pictures/what it looks
  // like>" is, and those words are already here.
  { kinds: ['individual_images'], re: /\b(images?|pictures?|photos?|thumbnails?|render|visuali[sz]ed?|appearance|morphology|looks? like|which (?:connectomes?|datasets?|templates?)|where (?:do|can|would) (?:i|we|you) find)\b/i },
  ...
  // A FlyBase feature term (gene, allele, construct, combination) offers
  // FindStocks itself; on a neuron class the stocks come from the split-stocks
  // macro (maybeInjectSplitStocksStep), and this rule finds nothing, harmlessly.
  { kinds: ['stocks'], re: STOCKS_INTENT_RE },
 ...
 */
/**
 * Drop species and common-name words from a name before it is searched.
 *
 * SPECIES_STOPWORDS has existed for a while, but only inside toks() — the token
 * set used to SCORE candidates. The string actually sent to VFB never had them
 * removed, and for "fly" that is not merely noise, it is a hijack: searching VFB
 * for "fly brain" returns 224 documents, essentially all of them "… on
 * JRC_FlyEM_Hemibrain" individuals, because FlyEM carries "fly" and Hemibrain
 * carries "brain". The class `brain` (FBbt_00005095), which is the rank-1 hit for
 * "brain" alone, is absent from that result set entirely. So the term did not
 * resolve, no region was available, and a reviewer asking how many neurons are in
 * the fly brain was told VFB holds nothing — while being offered counts for the
 * asymmetrical body, which is what AB(L)/AB(R) on JRC_FlyEM_Hemibrain are.
 *
 * Returns '' when there is nothing to strip or when stripping would leave
 * nothing, so an ASCII name with no species word costs one comparison.
 */
export function stripSpeciesWords(name = '') {
  const raw = String(name || '')
  const kept = (raw.match(/[^\s]+/g) || []).filter(w => {
    const bare = w.replace(/[^A-Za-z0-9']/g, '').toLowerCase()
    if (!bare) return true
    return !SPECIES_STOPWORDS.has(bare) && !SPECIES_STOPWORDS.has(singular(bare))
  })
  if (kept.length === 0) return ''
  const out = kept.join(' ').replace(/\s{2,}/g, ' ').trim()
  return norm(out) === norm(raw) ? '' : out
}
...
export function nameVariants(name = '') {
  ...
  // Species words go first, for the same reason transliteration does: every later
  // rung is wasted while the word that hijacks the search is still in the string.
  // Singularising "fly brains" to "fly brain" does not help when "fly" is what
  // pulls the whole result set onto the JRC_FlyEM_Hemibrain template.
  const despecied = stripSpeciesWords(raw)
  if (despecied) {
    push(despecied)
    push(singularisePhrase(despecied))
  }
  ...
  // "hierarchy of Kenyon cell", "subtypes of the medulla": the planner copies
  // the relation into the term, and nothing above strips it (issue #48). The
  // relation is the question's; the term is what follows "of".
  const relational = stripRelationalPrefix(raw)
  if (relational) {
    push(relational)
    push(singularisePhrase(relational))
  }
  // "visual processing", "olfactory coding": a function the user named, not a
  // structure. FBbt names the structure "<modality> system"; the bare modality
  // is the last try. Last, because it is the most invasive rewrite here.
  for (const v of functionalVariants(raw)) push(v)
  return out
}

const RELATIONAL_PREFIX_RE = /^(?:the\s+)?(?:full\s+|complete\s+|whole\s+|entire\s+)?(?:hierarch(?:y|ies)|taxonomy|classification|sub-?types?|sub-?classes|sub-?divisions?|types?|kinds?|classes|parts?|components?|list|tree|lineage tree|breakdown|structure)\s+(?:of|under|below|for)\s+(?:the\s+)?/i
/** "hierarchy of Kenyon cell subtypes" → "Kenyon cell"; '' when there is no prefix. */
export function stripRelationalPrefix(name = '') {
  const raw = String(name || '').trim()
  if (!RELATIONAL_PREFIX_RE.test(raw)) return ''
  return raw.replace(RELATIONAL_PREFIX_RE, '').replace(/\s+(?:sub-?types?|sub-?classes|types?|kinds?|parts?|components?)$/i, '').trim()
}

// --- spelling rung -----------------------------------------------------------
//
// "What does the anterior optic tubercule look like?" was answered with "could
// not be matched to a specific term in the VFB database" (2026-09-07 feedback,
// rated wrong). The class is anterior optic tubercle, FBbt_00007059. Nothing in
// FBbt is called "tubercule": it is a misspelling, not a synonym, and VFB's Solr
// stems the two words differently ("tubercul" / "tubercl"), so the search
// returned zero documents for the name and for every variant of it — each rung
// above rewrites something other than the misspelt word. The synthesiser then
// spotted the correct spelling in its own prose and sent it to the user, because
// it cannot search.
//
// This rung does what the synthesiser could not. A misspelt word cannot be
// searched for, but the REST of the name can: "anterior optic" returns the whole
// AOTU family with the class itself at rank 7. So each probe is the name with one
// word removed, and a document from a probe is accepted only when its label (or a
// synonym) has exactly the same words as the name except for ONE, and that one is
// within a couple of edits of the word it replaces. That is the same "exact or
// nothing" discipline the variant rungs keep — it cannot fire on a name the
// ladder already matched, and it cannot substitute a plausible neighbour for a
// term: "anterior optic tract" has the same three-word shape, but "tract" is five
// edits from "tubercule" and is refused.
//
// Bounded on purpose: at most SPELLING_PROBE_MAX probes, only words of
// SPELLING_MIN_WORD or more letters are treated as candidates for having been
// misspelt (a wrong short word — "of", "the" — leaves nothing to match on), and a
// single-word name has no rest to search, so it gets no probe at all.

const SPELLING_PROBE_MAX = 3
const SPELLING_MIN_WORD = 4

/** Levenshtein edit distance, bounded early: returns max+1 once it exceeds max. */
...
/** How many edits a word of this length may be from its intended spelling. */
const spellingTolerance = (word) => (word.length >= 7 ? 2 : 1)

/**
 * The searches to run for a name that may carry one misspelt word: the name
 * with each candidate word removed, longest word first (a long word is both
 * the likelier to be mistyped and the more distinctive to lose), capped.
 */
export function spellingProbes(name = '') {
  const words = String(name || '').trim().split(/\s+/).filter(Boolean)
  if (words.length < 2) return []
  const idx = words
    .map((w, i) => ({ i, len: w.replace(/[^A-Za-z]/g, '').length }))
    .filter(({ len }) => len >= SPELLING_MIN_WORD)
    .sort((x, y) => y.len - x.len || x.i - y.i)
    .slice(0, SPELLING_PROBE_MAX)
  const out = []
  for (const { i } of idx) {
    const probe = words.filter((_, j) => j !== i).join(' ')
    if (probe && !out.includes(probe)) out.push(probe)
  }
  return out
}

/**
 * The id of the document whose label or synonym is `queryName` with exactly one
 * word misspelt, or null. Every other word must match (after the same
 * singularising tokenisation the ladder uses), the word counts must agree, and
 * the differing pair must be within spellingTolerance edits of each other and
 * at least SPELLING_MIN_WORD letters long. Closest pair wins; VFB's own ranking
 * breaks ties.
 */
export function spellingMatchId(search, queryName = '') {
  const qTok = toks(queryName)
  if (qTok.length < 2) return null
  let best = null
  for (const d of validSearchDocs(search)) {
    const names = [docLabel(d), ...docSynonyms(d)]
    for (const candidate of names) {
      const cTok = toks(candidate)
      if (cTok.length !== qTok.length) continue
      const cLeft = [...cTok]
      const qOnly = []
      for (const t of qTok) {
        const k = cLeft.indexOf(t)
        if (k >= 0) cLeft.splice(k, 1); else qOnly.push(t)
      }
      if (qOnly.length !== 1 || cLeft.length !== 1) continue
      const [q] = qOnly; const [c] = cLeft
      if (q.length < SPELLING_MIN_WORD || c.length < SPELLING_MIN_WORD) continue
      const tol = Math.min(spellingTolerance(q), spellingTolerance(c))
      const dist = editDistance(q, c, tol)
      if (dist === 0 || dist > tol) continue
      if (!best || dist < best.dist) best = { id: sfOf(d), dist, as: candidate }
    }
  }
  return best
}
...
//
// The prime is part of the token. Mushroom body nomenclature separates sibling
// classes by nothing else — "alpha/beta Kenyon cell" (FBbt_00100248) and
// "alpha'/beta' Kenyon cell" (FBbt_00100249) — and a tokeniser that drops
// punctuation reads both as [alpha, beta, kenyon, cell], so the singularised
// exact-match stage returned whichever sibling VFB happened to rank first
// (issue #40). Curly quotes are folded to the ASCII prime first; a possessive
// "'s" is not a prime and is dropped.
const singular = (w) => (w.length > 3 && w.endsWith('s') ? w.slice(0, -1) : w)
const toks = (s) => (norm(s).replace(/[\u2019\u2018`\u00b4\u2032]/g, "'").replace(/'s\b/g, '').match(/[a-z0-9']+/g) || [])
  .map(w => w.replace(/^'+/, ''))
  .filter(Boolean)
  .map(singular)
  .filter(w => !SPECIES_STOPWORDS.has(w))
...
/**
 * Has this conversation actually SEEN this id — in the question, a prior turn,
 * a resolved term, or the registry that every search and query row is written
 * into? A step whose id fails this test was filled in from the planner's
 * memory, and a model's memory of ontology ids is a memory of their shape.
 */
export function idIsSourced(ledger, deps, id) {
  const wanted = shortId(id).toLowerCase()
  if (!wanted) return false
  const hit = (s) => String(s || '').toLowerCase().includes(wanted)
  if (hit(ledger.question)) return true
  if (Object.values(ledger.terms || {}).some(t => hit(t?.id) || hit(t?.fetchedId))) return true
  if (Object.values(ledger.registry || {}).some(r => hit(r?.id))) return true
  if (Array.isArray(deps?.history) && deps.history.some(m => hit(m?.content))) return true
  return false
}

// --- question-kind guard -----------------------------------------------------

const NEURON_KIND_QUESTION_RE = /\b(neurons?|neurones?|interneurons?|motor\s*neurons?|cells?|cell\s+types?|nblast|morpholog\w*|similar(?:ity)?|connect\w*|synap\w*|inputs?|outputs?|partners?|projections?|arbou?rs?|dendrit\w*|axons?)\b/i

/**
 * The kind of thing a question is about, when its wording says so: 'neuron'
 * for a question that talks about neurons, cells, morphology or wiring; null
 * otherwise. Deliberately one kind. The guard it drives re-searches within the
 * kind when the resolver's pick is outside it, and a wrong pick is silently
 * fatal only where every downstream tool accepts the id and returns nothing —
 * which is the neuron tools. A region question that resolved to a neuron class
 * still gets the class's queries and a visible answer.
 */
export function questionKindHint(question = '') {
  return NEURON_KIND_QUESTION_RE.test(String(question || '')) ? 'neuron' : null
}

const docFacets = (d) => (Array.isArray(d?.facets_annotation) ? d.facets_annotation : []).map(norm)
/** A search document whose facets say it is an individual (one record), not a class. */
export function docIsIndividual(d) {
  const f = docFacets(d)
  return f.includes('individual') && !f.includes('class')
}
const ENTITY_NOUN_TAIL_RE = /\s+(?:neurons?|neurones?|interneurons?|motor\s*neurons?|cells?|cell\s+types?|types?|classes|glia|glial\s+cells?)\s*$/i
/**
 * "EPG neurons", "the EPG neuron", "Kenyon cell types" — a symbol with a
 * category noun after it is a name for a TYPE, not for one record. A record's
 * own label never ends that way ("EPG-5L#3 (FAFB:4087066)"), so a name of
 * this shape that lands on an individual has landed on an example of the
 * type, not on what was asked for.
 */
export function namesAType(name = '') {
  return ENTITY_NOUN_TAIL_RE.test(String(name || '').trim())
}
/** "EPG neurons" → "EPG"; '' when nothing is left. */
export function stripEntityNoun(name = '') {
  return String(name || '').trim().replace(ENTITY_NOUN_TAIL_RE, '').trim()
}
/** A search document whose facets say it is a neuron (class or individual). */
export function docIsNeuron(d) {
  const f = docFacets(d)
  return f.includes('neuron') || f.includes('motor_neuron') || f.includes('sensory_neuron')
}

/**
 * Other documents in the same search that carry every word of the name, when
 * the name matched none of them exactly — the readings the resolver did NOT
 * take. Empty when the name was an exact match (there is nothing to disclose)
 * or when it was a phrase specific enough to have one superset. Labels only,
 * in the search's own order, capped at four.
 */
export function sameKindAlternatives(search, queryName = '', chosenId = '', cap = 4) {
  if (!chosenId || exactTermMatchId(search, queryName)) return []
  const qTok = toks(norm(queryName))
  if (!qTok.length) return []
  const chosen = validSearchDocs(search).find(d => sfOf(d) === chosenId)
  const chosenIsNeuron = chosen ? docIsNeuron(chosen) : false
  const out = []
  for (const d of validSearchDocs(search)) {
    if (sfOf(d) === chosenId) continue
    if (chosen && docIsNeuron(d) !== chosenIsNeuron) continue
    const lt = nameToks(docLabel(d))
    if (!qTok.every(t => lt.includes(t))) continue
    const label = docLabel(d)
...
// Words that describe what a structure DOES rather than name it. "visual
// processing", "olfactory coding", "motor control": the second word is the
// user's, the first is the ontology's. Stripping the function word leaves the
// modality, and "<modality> system" is FBbt's own name for the anatomical
// system that does it (visual system, olfactory system, auditory system,
// gustatory system, mechanosensory system).
const FUNCTION_WORDS = new Set(['processing', 'perception', 'sensation', 'sensing', 'detection', 'coding', 'encoding', 'integration', 'computation', 'function', 'functions', 'behaviour', 'behavior', 'control', 'circuit', 'circuits', 'circuitry', 'pathway', 'pathways', 'signalling', 'signaling', 'transduction', 'recognition', 'discrimination', 'learning', 'memory'])
const isFunctionWord = (w) => FUNCTION_WORDS.has(norm(w))

/** "visual processing" → ["visual system", "visual"]; [] when no function word. */
export function functionalVariants(name = '') {
  const words = String(name || '').trim().split(/\s+/).filter(Boolean)
  if (!words.some(isFunctionWord)) return []
  const core = words.filter(w => !isFunctionWord(w) && !/^(?:the|of|in|for|and)$/i.test(w)).join(' ').trim()
  if (!core) return []
  return [`${core} system`, core]
}

/** How many content words a loose search may spend a search on. */
const LOOSE_WORD_CAP = 2
/** How many candidates each loose word contributes. */
const LOOSE_PER_WORD = 3

/**
 * Terms VFB does hold that share a content word with a name it could not
 * match. Anatomy only — a bare word like "visual" also matches hundreds of
 * gene and clone records, and those are not things to rephrase a
 * neuroanatomy question around. Never a reading of the phrase; the caller
 * marks these LOOSE and the prompt says so.
 */
async function looseWordCandidates(name, deps, budget) {
  const words = toks(name)
    .filter(t => t.length >= 4 && t !== norm(name) && !GENERIC_RESOLVE_WORDS.has(t) && !ENTITY_CATEGORY_WORDS.has(singular(t)) && !isFunctionWord(t))
  const out = []
  for (const word of words.slice(0, LOOSE_WORD_CAP)) {
    if (budget.expired()) break
    const raw = await raceDeadline(
      deps.runTool('vfb_search_terms', { query: word, rows: 10, minimize_results: false, filter_types: ['anatomy'] }),
      budget.remaining()
    )
    if (raw === TIMED_OUT) break
    for (const label of searchCandidateLabels(parseMaybe(raw), LOOSE_PER_WORD)) {
...
export function pickBestTermId(search, queryName = '') {
  ...
  if (q) {
    ...
    if (qTok.length) {
      ...
      if (cands.length) {
        // A one-word name is a SYMBOL ("DA1", "LC10"), and "the shortest label
        // containing it" is no longer a preference between stage qualifiers —
        // it is a coin toss weighted towards whichever unrelated class happens
        // to have the fewest words. "DA1" over the neuron classes picks
        // "larval DA1 motor neuron" (four words) above "adult antennal lobe
        // projection neuron DA1" (six). The search's own ranking, which put
        // the projection neuron first on its matched synonym "DA1 PN", is the
        // better judge of a symbol; the length rule stays for phrases.
        if (qTok.length > 1) {
