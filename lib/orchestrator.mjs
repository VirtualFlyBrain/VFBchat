...
async function resolveTerms(ledger, names, deps, models, log) {
  ...
  await Promise.all(names.map(async (name) => {
    ...
    if (!id) {
      ...
      addTerm(ledger, name, { id: null, attempted: true, candidates, truncated: ladderCutShort })
      log({ resolve: name, id: null, candidates: candidates.length, truncated: ladderCutShort })
      return
    }
