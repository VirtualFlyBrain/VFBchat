...
import { isIndividualImageQuery, querySemantics, isGeneExpressionQuestion, isDriverLineQuestion } from './queryTypes.mjs'
  ...
  if (plan.documentation) {
    ...
    for (const cand of pickDocCandidates(search, DOC_CANDIDATES)) {
      ...
      if (answered) {
        ...
        addEvidence(ledger, buildEvidenceRow({ source: 'doc', claim: ex.value.claim, verbatim: completeQuoteFromSource(ex.value.verbatim, page), locator: { url: cand.url, title: cand.title } }))
      }
...
async function ingestParallelDocs(ledger, docSearchPromise, deps, models, log) {
  ...
  try {
    ...
    if (ex.ok && ex.value?.relevant && ex.value.answered) {
      addEvidence(ledger, buildEvidenceRow({ source: 'doc', claim: ex.value.claim, verbatim: completeQuoteFromSource(ex.value.verbatim, page), locator: { url: top.url, title: top.title } }))
    }
...
async function synthesise(ledger, deps, models) {
  ...
  const docIntent = String(ledger.intent || '') === 'documentation'
  ...
  const docBlock = docAnswered
    ? '\n\nDOCUMENTATION ANSWERED THIS. The absence rule ("VFB does not currently hold …") is about VFB\'s DATA — ontology terms, connectivity, images — not about its website or documentation. Do NOT close with a disclaimer that VFB lacks documentation, instructions or details on this: a documentation page answered it. Answer from that page and stop. If the question asks HOW to do something, give the actual steps: the general no-URLs, no-ids rule is about ontology terms and about links you would have to invent, and it does NOT apply to a command, a configuration snippet or a service address that appears verbatim on the page you just read. Reproduce those exactly, in a code block, rather than pointing at the page that has them — WHOLE, never a fragment: a configuration the reader has to reassemble is no more use than none, so if you mention a JSON configuration, write out the entire block including its braces and enclosing keys, not the one line inside it that names the address. Transcribe it character for character as it appears on the page — do NOT re-indent it, re-wrap it or tidy it up. Reformatting is how the last brace goes missing, and a configuration that does not parse is worse than none because it looks complete. Before you finish the block, check that every brace and bracket you opened is closed. What you reproduce is the snippet the page shows the reader, never the evidence record that carries it: an object with "claim", "source", "verbatim" or "ref" keys is the shape of YOUR input and must never appear in the answer.'
    : ''
  ...
  // The pages this answer is being written from, for the in-stream fenced-block
  // repair. The instruction above gets the configuration block right most of the
  // time and not all of it: on some runs the model re-indents the JSON as it
  // copies and loses the last brace. Since the answer streams, there is no
  // afterwards in which to notice — so the sink is given the quotes and closes
  // the block from them as it goes. See lib/fencedBlockRepair.mjs.
...
export function pickBestTermId(search, queryName = '') {
  ...
  if (q) {
    ...
    const top = valid[0]
    const topToks = new Set([...toks(docLabel(top)), ...docSynonyms(top).flatMap(s => toks(s))])
