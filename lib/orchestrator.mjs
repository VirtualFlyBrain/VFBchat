...
async function resolveTerms(ledger, names, deps, models, log) {
  ...
  await Promise.all(names.map(async (name) => {
    ...
    try {
      ...
      } else if (isTermInfo(fetched)) {
        const d = buildTermInfoDigest(fetched)
        fetchedId = d.id || 'no-id'
        // Guard: if get_term_info returns a DIFFERENT term than requested, discard
        // the digest so its name/queries/counts can't poison the answer — the
        // reliable search-derived label is used instead.
