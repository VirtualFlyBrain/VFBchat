...
async function synthesise(ledger, deps, models) {
  ...
  // Surface resolved terms + their VFB ids so the synthesiser can render the
  // canonical links the UI turns into thumbnails (richness the evidence rows alone
  // don't carry).
  const terms = Object.entries(ledger.terms)
    .filter(([, t]) => t.id)
    .map(([name, t]) => ({ name, id: t.id, label: t.label || name }))
  const historyBlock = Array.isArray(deps.history) && deps.history.length
    ...
    : ''
  // Thumbnail URLs harvested from tool outputs. Embedding the relevant ones as
  // markdown images is how the UI renders thumbnails (richness the evidence rows
  // don't carry); the wiring also attaches them as a fallback if omitted.
  const imageHints = Array.isArray(deps.imageHints) ? deps.imageHints.slice(0, 12) : []
  const imagesBlock = imageHints.length
    ? `\n\nIMAGE THUMBNAILS available from the tool data (include the ones relevant to the answer, verbatim, as markdown images so they render):\n${imageHints.map(u => `![image](${u})`).join('\n')}`
    : ''
  const messages = [
    { role: 'system', content: `You are a Virtual Fly Brain assistant. Write the answer using ONLY the supplied evidence. Distinguish a paper's claim ("literature") from a VFB-database fact ("vfb") and from documentation ("doc") — never present a paper's claim as a VFB-database fact, and cite papers inline. Do NOT append per-sentence source tags such as "(vfb)", "(source: vfb)" or "according to the VFB database" — provenance is shown separately to the user as linked sources. Do NOT write raw ontology ids such as FBbt_00005801 or VFB_xxxx in the prose; refer to entities by their name only. Include relevant thumbnail images verbatim as markdown when provided. If evidence is missing for part of the question, say plainly what VFB does and does not have rather than guessing.` },
    { role: 'user', content: `${historyBlock}QUESTION:\n${ledger.question}\n\nRESOLVED TERMS (JSON):\n${JSON.stringify(terms)}\n\nEVIDENCE (JSON):\n${JSON.stringify(evidence)}${imagesBlock}\n\nWrite the answer.` }
  ]
