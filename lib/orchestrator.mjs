...
async function resolveTerms(ledger, names, deps, models, log) {
  ...
  await Promise.all(names.map(async (name) => {
    ...
    try {
      ...
      if (isTermInfo(fetched)) {
        const d = buildTermInfoDigest(fetched)
        // Guard: VFB get_term_info sometimes returns a DIFFERENT/related term than
        // requested (e.g. the cell-body-rind for a region). If the payload's id
        // doesn't match, discard the digest so its name/queries/counts can't poison
        // the answer — the reliable search-derived label is used instead.
