...
import { isTermInfo, buildTermInfoDigest, termInfoToDigestText, unwrapTermInfo, parseReplacedBy, isDeprecatedRecord, PREVIEW_COUNT_CAP } from './termInfoDigest.mjs'
...
async function resolveTerms(ledger, names, deps, models, log) {
  ...
  await Promise.all(names.map(async (name) => {
    ...
    if (!id) { addTerm(ledger, name, { id: null, attempted: true }); log({ resolve: name, id: null }); return }
