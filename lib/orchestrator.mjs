...
export async function runHarness(question, deps) {
  ...
  while (guard++ < hardCap) {
    ...
    if (action.action === 'resolve_terms') {
      await resolveTerms(ledger, action.terms, deps, log)
      continue
    }
...
async function resolveTerms(ledger, names, deps, log) {
  // Fan out search_terms in parallel; pick the best id deterministically, then
  // fetch term info for publications (enables the literature escalation).
  await Promise.all(names.map(async (name) => {
    ...
    let publications = []
    try {
      const info = parseMaybe(await deps.runTool('vfb_get_term_info', { id }))
      publications = extractPublicationRefs(info || {})
    } catch { /* term info optional */ }
    addTerm(ledger, name, { id, publications })
...
async function runStep(ledger, step, deps, paramsByName, models, log) {
  ...
  let out = await deps.runTool(step.tool, args)
