...
import { isTermInfo, buildTermInfoDigest, termInfoToDigestText, unwrapTermInfo } from './termInfoDigest.mjs'
...
async function resolveTerms(ledger, names, deps, models, log) {
  ...
  await Promise.all(names.map(async (name) => {
    ...
    try {
      ...
      const record = (fetched && fetched.error) ? null : unwrapTermInfo(fetched, id)
      if (fetched && fetched.error) {
        ...
        // Safety net: if the returned record's id still doesn't match the request,
        // discard the digest so a wrong term can't poison the answer.
        if (d.id && shortId(d.id) !== shortId(id)) {
          info = null
        } else {
          info = record
          digest = d
    ...
    addTerm(ledger, name, { id, publications, info, digest, fetchedId })
    ...
    const mismatched = /^(FBbt|VFB|FBgn|FBal|FBti|FBtp|FBco)_/.test(fid) && shortId(fid) !== shortId(id)
    if (/^(error|threw|null)/.test(fid) || mismatched) {
      console.error(`[VFBchat] get_term_info FAILED | term="${name}" requested_id=${id} returned=${fid} | digest=${digest ? 'kept' : 'discarded'}`)
    }
...
export function pickBestTermId(search, queryName = '') {
  ...
  const toks = (s) => (norm(s).match(/[a-z0-9]+/g) || []).map(singular)
