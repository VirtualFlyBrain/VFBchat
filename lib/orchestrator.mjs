...
export function pickBestTermId(search, queryName = '') {
  ...
  if (q) {
    ...
    // 3. stage-prefixed exact match: VFB labels are often stage-qualified, so
    //    "lateral horn" should bind to "adult lateral horn", not a fuzzy hit.
    const stagePrefixed = valid.find(d => /^(adult|larval|embryonic|pupal|adult |embryonic\/larval)\s+/.test(norm(d.label)) && norm(d.label).replace(/^(adult|larval|embryonic|pupal|embryonic\/larval)\s+/, '') === q)
    if (stagePrefixed) return sfOf(stagePrefixed)
    // 4. shortest label that CONTAINS the term as a whole phrase. Prefers the
    //    general region ("adult lateral horn") over a containing-phrase neuron
    //    ("adult lateral horn Leucokinin neuron") — the recurring mis-resolution.
    const phrase = new RegExp(`(^|[^\\w-])${escapeForRe(q)}([^\\w-]|$)`)
    const containing = valid.filter(d => phrase.test(norm(d.label)))
    if (containing.length) {
      containing.sort((a, b) => norm(a.label).length - norm(b.label).length)
      return sfOf(containing[0])
    }
  }
  // 5. fall back to the top-ranked valid id (Solr already boosts FBbt/VFB)
  return sfOf(valid[0])
}
