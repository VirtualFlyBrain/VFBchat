  ...
  await Promise.all(names.map(async (name) => {
    ...
    if (info) {
      const ex = await deps.callStructured({
        messages: buildVfbExtractMessages(ledger.question, [ledger.question], compactForExtract(info), 'vfb_get_term_info'),
...
async function runStep(ledger, step, deps, paramsByName, models, log) {
  ...
  const slice = compactForExtract(parsed ?? out)   // all fields preserved, sampled+capped
  const ex = await deps.callStructured({
...
// Compact a result for an extractive call WITHOUT deciding which fields matter:
// every key is preserved; only long strings and long arrays are sampled, and a
// hard char cap applies. The extractive LLM — seeing the question and the whole
// (bounded) result — decides what is relevant. For very large results the proper
// mechanism is the data_resource handle (inspect/read/search) so the model can
// navigate the full data; this compaction is the in-context fallback.
function compactForExtract(value, maxChars = MAX_EXTRACT_CHARS) {
  const compact = compactValue(value, 0)
  let s = typeof compact === 'string' ? compact : asText(compact)
...
function compactValue(v, depth) {
  ...
  if (Array.isArray(v)) return v.slice(0, 12).map(x => compactValue(x, depth + 1))
  if (typeof v === 'object') {
    if (depth > 7) return '…'
    const o = {}
    for (const [k, val] of Object.entries(v)) o[k] = compactValue(val, depth + 1)
    return o
  }
  return v
}
