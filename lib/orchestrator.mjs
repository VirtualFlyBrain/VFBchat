...
import { maybeInjectSufficiencyQueries } from './sufficiency.mjs'
...
async function synthesise(ledger, deps, models) {
  ...
  // This block used to read as a permission ("use to say what IS available"),
  // and a permission loses to a rule. The absence rule below is imperative; a
  // model weighing "never claim no information when this lists relevant data"
  // against "say VFB does not currently hold X" resolves in favour of the
  // sentence that reads like an instruction. That is how an answer denied the
  // mushroom body's input and output neurons while this very block was
  // advertising 366 presynaptic and 304 postsynaptic records for it. It is the
  // same confusion as VFBquery's -1: a count VFB holds against a query nobody
  // ran is not a result of zero. So the block now states what these numbers are
  // (holdings, not results) and forbids the denial outright.
  const availableBlock = availableData.length
    ? `\n\nAVAILABLE VFB DATA for the resolved terms — records VFB HOLDS, whose queries HAVE NOT BEEN RUN in this session:\n${availableData.join('\n')}\n`
      + 'These are holdings, not results. A query that was not run is NOT an empty query: you are FORBIDDEN to write "VFB does not currently hold data on …", "no data is available for …", or any other absence about anything this list covers, however the question was worded. Where the question asks for something this list has, say VFB holds it, give the count, and say the query has not been run.'
    : ''
...
const QUERY_INTENT_RULES = [
  ...
  { kinds: ['class_list'], re: CONNECTIVITY_INTENT_RE, preferAll: ['NeuronsPresynapticHere', 'NeuronsPostsynapticHere'] },
...
function injectIntentQuerySteps(ledger, question, t, log = () => {}) {
  ...
  const planned = new Set(ledger.plan
    ...
    .map(s => `${String(s.args.id || '')}::${String(s.args.query_type).toLowerCase()}`))
