...
async function resolveTerms(ledger, names, deps, models, log) {
  ...
  await Promise.all(names.map(async (name) => {
    ...
    // Plural retry. VFB's index does not stem multi-word phrases, so "visual
    // system neurons" scores ZERO where "visual system neuron" scores one, and
    // zero hits means not even a candidate list to offer the user — the harness
    // abstains outright on a term VFB holds. Only fires when the first search
    // returned literally nothing, so it can neither change nor reorder a search
    // that did find something; at worst it costs one extra call on a name that
    // was going to fail anyway. Matching stays against the ORIGINAL name:
    // pickBestTermId already singularises tokens (stages 2a/2b), and the plural
    // is what tells it "lateral horn neurons" means the region's result set.
    if (!directId && searchIsEmpty(search)) {
      const singular = singularisePhrase(name)
      if (singular) {
        const retry = parseMaybe(await deps.runTool('vfb_search_terms', { query: singular, rows: 30, minimize_results: false }))
        if (!searchIsEmpty(retry)) {
          search = retry
          log({ resolve_retry: name, as: singular })
        }
      }
    }
