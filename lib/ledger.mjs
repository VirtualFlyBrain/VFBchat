...
export function setPlan(ledger, plan) {
  ...
  ledger.plan = (Array.isArray(plan?.steps) ? plan.steps : []).map((s, i) => ({
    ...
    // Same reasoning as via_template: a chip step's {id, query_type} came from
    // the CLIENT, so it is the other kind of step that may have to be withdrawn
    // once the term's real catalogue is known.
    ...(s.via_chip ? { via_chip: true } : {})
  }))
