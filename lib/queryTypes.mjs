...
const KIND_CUES = [
  ...
  ['connectivity', /\bconnect\w*|\bpartners?\b|\bupstream\b|\bdownstream\b|\bpre-?synap\w*|\bpost-?synap\w*|\bsynap\w*|\binputs?\b|\boutputs?\b|\btargets?\b|\bprojects?\s+to\b|\bcircuits?\b|\bafferent\w*|\befferent\w*/i],
  ...
  ['dataset', /\bdata\s?sets?\b|\bconnectomes?\b|\bem\s+volumes?\b/i],
...
export function questionKinds(question = '') {
  ...
  for (const [kind, re] of KIND_CUES) if (re.test(q)) out.add(kind)
