...
export const QUERY_SEMANTICS = {
  ...
  // split-GAL4 combinations. A separate kind from `expression` on purpose: a
  // TransgeneExpressionHere row is any reported transgene expression pattern —
  // overwhelmingly single GMR enhancer-fragment GAL4 lines — and a split-GAL4 is
  // an intersection of two hemidrivers. Production answered "what split-GAL4
  // lines label the lateral horn?" with TransgeneExpressionHere rows and called
  // P{GMR11B09-GAL4} a split-GAL4, five times out of five. It is not one, and
  // conflating the two kinds is what let that happen.
  //
  // SplitsTargeting was missing from this table entirely, so querySemantics
  // returned kind 'other', no intent rule mentions 'other', and
  // pickQueriesByIntent could never select it on any term.
  SplitsTargeting: S('splits', 'split-GAL4 combinations', 'split-GAL4 hemidriver combinations that target a neuron type'),
...
// A split-GAL4 is an intersection of two hemidrivers (an AD and a DBD), and VFB
// records them under their own query, SplitsTargeting, on NEURON CLASSES. It is
// a narrower thing than "a driver line", and asking for one and being handed a
// list of single GMR GAL4 lines is a wrong answer, not an approximate one.
// "split drivers", "split hemidrivers" and a bare "split" before the reagent
// nouns are the same request (issue #46: "split drivers" matched nothing here,
// so the question fell through to the general driver-line rule).
const SPLIT_GAL4_RE = /\bsplit[-\s]?gal4s?\b|\bsplit[-\s]?(?:lines?|drivers?|combinations?|reagents?)\b|\bsplits\b|\bhemi[-\s]?drivers?\b|\b(?:ad|dbd)[-\s]?hemi/i

/** True for a question specifically about split-GAL4 combinations. */
export function isSplitGal4Question(question = '') {
  return SPLIT_GAL4_RE.test(String(question || ''))
}
...
const KIND_CUES = [
  ['individual_images', /\bimages?\b|\bpictures?\b|\bthumbnails?\b|\bimage stacks?\b|\bindividual\b|\blooks? like\b|\b3d\b|\bvisuali[sz]\w*/i],
  ...
  ['splits', SPLIT_GAL4_RE],
  ['stocks', /\bstocks?\b|\bbloomington\b|\bbdsc\b|\bfly\s+lines?\b/i],
