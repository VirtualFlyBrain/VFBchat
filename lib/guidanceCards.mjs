...
export const CARDS = [
  ...
  {
    id: 'scrnaseq',
    match: (q) => /\b(scrna-?seq|single[- ]cell|transcriptom\w*|gene expression|which genes|what genes|receptor genes?|express(?:es|ed)? .*(?:gene|receptor)|dopamine receptor|acetylcholine receptor)\b/i.test(q),
    ...
    synth: 'For gene-expression answers, report the expression level and the fraction of cells expressing (extent) per cluster/subtype where available, cite the scRNA-seq dataset publication, and attribute it to that dataset — do not present single-cell expression as settled biology.'
  },
  {
    id: 'neurotransmitter',
...
// Complexity router. The voted planner (3 ELM calls) and a deep tool budget cost
// real latency that a simple definitional lookup does not need. Classify the
// question off the same card matchers and scale effort to it:
...
//   - simple (no card — term-info / definitional): one planner vote, shallow budget.
...
export function classifyComplexity(question = '') {
  ...
  return { tier: 'simple', plannerVotes: 1, maxToolRounds: 10 }
}
