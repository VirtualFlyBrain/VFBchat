...
export const CARDS = [
  ...
  // question on the word "split" and send the planner to TransgeneExpressionHere.
  {
    ...
    // The planner card does not reach the synthesiser, and the synthesiser is
    // what writes the sentence. Without this line the relabelling came back in
    // roughly one run in three: the prose still called P{GMR…-GAL4} rows
    // split-GAL4 lines even when the plan was right.
    //
    // Positive framing with a worked example, not a bare prohibition — measured
    // at 20/20 against 16/20 for prohibitions alone on the rule this project
    // already ablated.
    synth: 'SPLIT-GAL4 NAMING: a transgene expression row is an expression pattern, not a split-GAL4 line. P{GMR11B09-GAL4} and PBac{…} rows are single driver lines. Write "VFB holds N transgene expression reports for <term>, including driver lines such as P{GMR11B09-GAL4}" — and, if the question asked for splits, add that VFB does not record split-GAL4 combinations for this term. Do NOT write "these include split-GAL4 lines such as P{GMR11B09-GAL4}".'
  },
  {
    id: 'genetic-tools',
  ...
  // planner to the region's own subclasses.
  {
    ...
    synth: 'INTRINSIC NEURONS: answer with the subclasses of the "<region> intrinsic neuron" class in the evidence (for the mushroom body these are the Kenyon cells and their subtypes). Output, input and modulatory neurons of a region — MBONs, DANs, APL, projection neurons — are EXTRINSIC and must not be named as intrinsic, whatever a paper snippet says about them; a VFB ontology fact outranks a literature snippet here.'
  },
  {
    id: 'taxonomy',
  ...
  },
  {
    ...
    planner: 'FLY STOCKS: for a NEURON CLASS ("stocks for the splits that label X", "which lines can I order for X") use vfb_find_split_stocks with the class name — it runs SplitsTargeting and then FindStocks on each split\'s two hemidriver constructs and reports the stock carrying both. For a FlyBase feature (gene/allele/insertion/combination) use vfb_find_stocks; the id pattern fixes the type: FBgn = gene (find stocks via its alleles), FBal = allele, FBti = insertion, FBtp = construct, FBco = split-GAL4 combination. Never pass an anatomy term to vfb_find_stocks.',
    synth: 'STOCKS: name each split-GAL4 line with its stock centre and stock number exactly as the evidence gives them (e.g. "BDSC 68318"). A split with no combined stock is reported as "no stock carrying both hemidrivers is recorded in FlyBase", not as VFB lacking stock data.'
  },
  {
    id: 'neuron-profile',
