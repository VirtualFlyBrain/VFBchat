...
export function planRetrieval({ question = '', vfbAnswered = false, vfbHasData = true, hasRefs = false } = {}) {
  const q = String(question)
  const explicit = EXPLICIT_EXTERNAL.test(q)
  const reasons = []

  // VFB fully answered and nothing external requested → no escalation.
  if (vfbAnswered && vfbHasData && !explicit) {
  ...
  if (needsDocumentation(q)) { documentation = true; reasons.push('doc-intent') }
  if (!vfbAnswered && needsLiterature(q, hasRefs)) { literature = true; reasons.push('lit-gap') }
  if (EXPLICIT_LIT.test(q)) { literature = true; reasons.push('explicit-literature') }

  // Completeness fallback: VFB gave nothing usable → check external sources rather than dead-stop.
  if (!vfbHasData) {
    literature = true
    reasons.push('vfb-empty-fallback')
  }
