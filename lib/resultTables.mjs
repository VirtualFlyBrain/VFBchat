...
import { isGeneExpressionQuestion, questionKinds, querySemantics } from './queryTypes.mjs'
...
export function buildTables(ledger, question = '', { maxTables = 2 } = {}) {
  ...
  // The KIND the question asks for, in the vocabulary the queries are typed
  // with (lib/queryTypes.mjs). Label overlap alone tied "Subclasses of Kenyon
  // cell" with "List all available images of Kenyon cell" on "show me the
  // hierarchy of Kenyon cell subtypes" — and the tie went to the bigger table,
  // 32,328 images (issue #48). A kind match outranks a word match.
  const wantedKinds = questionKinds(question)
  const candidates = []
  for (const t of resolved) {
    ...
    for (const q of t.digest.queries) {
      ...
      if (kind && wantedKinds.has(kind)) score += 3
