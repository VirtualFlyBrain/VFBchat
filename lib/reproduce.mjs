...
import { getTermId } from './ledger.mjs'
 ...
 */
/**
 * catalogue query type -> the VFBTerm property that answers it.
 *
 * Matched against each property's own docstring in VFB_connect, against VFB's
 * `use` string in queryTypes.mjs — never against the two names looking alike.
 * `NeuronsPresynapticHere` -> `.downstream_neuron_types` is the case that makes
 * the rule worth stating: the names disagree, the definitions are identical.
 *
 * Deliberately smaller than VFBQUERY_FUNCTIONS. A query type whose VFB_connect
 * counterpart is close-but-not-equal (AlignedDatasets vs `.datasets`,
 * ComponentsOf vs `.children`, LineageClonesIn vs clones-or-clone-types) is
 * absent rather than approximated, and falls through to VFBquery. Handing a user
 * a line that returns nearly the right rows is worse than handing them none.
 */
// A python identifier from a catalogue query type: NeuronsPartHere ->
// neurons_part_here. Deduped, because a run can use the same query type for two
// different terms. Constants are SHOUTED and results are lower case, so the two
// can never collide however a term happens to be named.
function resultVar(queryType, used) {
  const base = String(queryType || 'result')
    .replace(/([a-z0-9])([A-Z])/g, '$1_$2')
    .replace(/[^A-Za-z0-9]+/g, '_')
    .replace(/^_+|_+$/g, '')
    .toLowerCase() || 'result'
  let name = /^[0-9]/.test(base) ? `q_${base}` : base
  let n = 2
...
export const VFB_CONNECT_PROPERTIES = {
  // ontology
  SubclassesOf: 'subtypes',
  PartsOf: 'subparts',
  // region membership
  ...
  // reagents
  TransgeneExpressionHere: 'transgene_expression',
  // similarity
  SimilarMorphologyTo: 'similar_neurons_nblast',
  // scRNAseq
  anatScRNAseqQuery: 'scRNAseq_expression',
  clusterExpression: 'scRNAseq_genes'
}

// "in vfb_connect", "with VFB-connect", "using vfb connect". Asking for
// VFB-connect by name is the only thing that switches idiom: VFBquery is the
// default because it is the one the catalogue maps completely.
const VFB_CONNECT_RE = /\bvfb[ _-]?connect\b/i

export function wantsVfbConnect(question = '') {
  return VFB_CONNECT_RE.test(String(question || ''))
}

/**
 * The same run, rendered as VFB_connect. Returns null when nothing in the run
 * has a property we can vouch for, so the caller falls back to VFBquery rather
 * than emitting an import and no work.
 */
export function vfbConnectSnippet(reproduction) {
  if (!reproduction) return null
  const calls = (reproduction.calls || [])
    .filter(c => VFB_CONNECT_PROPERTIES[c.query_type])
  if (!calls.length) return null

  const varOf = {}
  const used = new Set()
  const lines = []
  lines.push('# Reproduce this answer in Python — pip install vfb-connect')
  lines.push('#')
  lines.push('# These are the exact ids this conversation resolved, so this returns the same')
  lines.push('# rows the answer was written from rather than a fresh search that might land')
  lines.push('# on a different term.')
  lines.push('from vfb_connect import vfb')
  lines.push('')

  for (const t of (reproduction.ids || [])) {
    if (!calls.some(c => c.id === t.id)) continue
    let name = String(t.label || t.id).toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '')
    if (!name || /^[0-9]/.test(name)) name = `term_${name}`
    let candidate = name, n = 2
    while (used.has(candidate)) candidate = `${name}_${n++}`
    used.add(candidate)
    varOf[t.id] = candidate
    lines.push(`${candidate} = vfb.term('${t.id}')  # ${t.label}`)
  }
  lines.push('')

  const usedResults = new Set(used)
  for (const c of calls) {
    const v = varOf[c.id]
    if (!v) continue
    lines.push(`# ${c.query_type} for ${c.label}`)
    lines.push(`${resultVar(c.query_type, usedResults)} = ${v}.${VFB_CONNECT_PROPERTIES[c.query_type]}`)
  }
  return lines.join('\n')
}
 ...
 */
// The registry as a second lookup for a "$term:NAME" placeholder. `getTermId`
// reads `ledger.terms`, which holds what THIS turn resolved; the registry holds
// what the conversation has established, and a template-routed step can name a
// term the current turn never had to re-resolve.
function registryId(ledger, name) {
  const wanted = String(name || '').trim().toLowerCase()
  if (!wanted) return ''
  for (const e of Object.values(ledger?.registry || {})) {
    if (e?.id && String(e.label || '').trim().toLowerCase() === wanted) return e.id
  }
  return ''
}
...
export function ranQueries(ledger) {
  const seen = new Set()
  const out = []
  const take = (id, qt, stepId, status) => {
    if (!qt || !id || !VFB_ID_RE.test(String(id))) return
    const key = `${id}::${qt}`
    if (seen.has(key)) return
    seen.add(key)
    out.push({ id: String(id), query_type: String(qt), stepId, status })
  }

  // Dispatched catalogue queries: the ordinary case.
  //
  // The id is read THROUGH the "$term:NAME" placeholder. A planned step keeps
  // the template it was written with and `resolveArgs` substitutes the real id
  // only at dispatch, so reading `args.id` raw sees "$term:Kenyon cell" — not a
  // VFB id, rejected, and the whole query silently absent from the working.
  //
  // That is the actual cause of C12, which three attempts guessed wrong about:
  // "What are the subclasses of Kenyon cell?" takes the TEMPLATE route, whose
  // step is planned as { id: '$term:Kenyon cell', query_type: 'SubclassesOf' }.
  // The query ran, returned 37 rows and answered the question; only the
  // reproduction could not see it. The follow-up turn resolved the id before
  // planning, which is why turn 2 always worked and turn 1 never did.
  for (const s of (ledger?.plan || [])) {
    if (s.tool !== 'vfb_run_query') continue
    const raw = s.args?.id
    const id = (typeof raw === 'string' && raw.startsWith('$term:'))
      ? (getTermId(ledger, raw) || registryId(ledger, raw.slice(6)))
      : raw
    take(id, s.args?.query_type, s.id, s.status)
  }

  // …and queries the run drew an answer from WITHOUT dispatching. A step whose
  // arguments could not be determined is answered from the term-info preview
  // instead (`via: 'digest'`), which is a real answer from real VFB data under a
  // named query type — so the working behind it is the same working, and the
  // user asking how to reproduce it should get that call. Plan steps come first
  // so a query that was both dispatched and previewed keeps its dispatched
  // status rather than the fallback's.
  for (const e of (ledger?.evidence || [])) {
    if (e?.via !== 'digest' || !e.query_type) continue
    take(e.id, e.query_type, e.stepId, 'satisfied')
  }
  return out
}
...
export function buildReproduction(ledger, { question = '', context = null, maxCalls = 8 } = {}) {
  ...
  if (calls.length) {
    ...
    for (const c of calls) {
      ...
      lines.push(`# ${noun}`)
      lines.push(`${resultVar(c.query_type, resultNames)} = ${c.python}`)
    }
  }
  if (unmapped.length) {
    lines.push('')
...
export function reproductionBlock(reproduction, question) {
  ...
  // Asked for VFB-connect by name, answered in VFB-connect — but only if this
  // run's queries have properties we can vouch for. Otherwise VFBquery, which
  // the catalogue maps completely, and which is the default either way.
  const body = (wantsVfbConnect(question) && vfbConnectSnippet(reproduction)) || reproduction.python
  return `\n\n\`\`\`python\n${body}\n\`\`\``
}
