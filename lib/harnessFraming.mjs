...
// EVIDENCE AS THE SUBJECT OF THE SENTENCE
//
// The noun rules above catch "the provided evidence" as an OBJECT — "not
// mentioned in the provided evidence". They miss the other half of the same
// habit, where the input is promoted to subject and made to do the reporting:
//
//   "the evidence does provide information on the strongest outputs of a neuron"
//
// Renaming that noun to "VFB evidence" would not help, because the sentence
// would still be about a document rather than about a fly. The claim itself is
// sound — those really are the strongest outputs — so this is a rewrite, not a
// cut: hand the verb back to VFB, which is the thing that actually records it.
//
// The negative forms are deliberately excluded. "The evidence does not provide
...
const EVIDENCE_SUBJECT_RULES = [
  ...
  [new RegExp(`\\b${EVIDENCE_SUBJECT}\\s+does\\s+(?:however\\s+)?provide\\s+(?:some\\s+)?(?:information|details|data|figures)\\s+(?:on|about|regarding|for|concerning)\\s+`, 'gi'), 'VFB records '],
  // "the evidence provides information on X" -> "VFB records X"
  [new RegExp(`\\b${EVIDENCE_SUBJECT}\\s+provides?\\s+(?:some\\s+)?(?:information|details|data|figures)\\s+(?:on|about|regarding|for|concerning)\\s+`, 'gi'), 'VFB records '],
  // "the evidence shows / indicates / states that X" -> "VFB records that X"
  [new RegExp(`\\b${EVIDENCE_SUBJECT}\\s+(?:does\\s+)?(?:however\\s+)?(?:shows?|indicates?|states?|reports?|records?|lists?)\\s+`, 'gi'), 'VFB records ']
]

/**
 * Connectives that only make sense with a sentence in front of them.
 *
 * Cutting the first sentence of an answer orphans whatever joined the second
 * one to it, and the join is the part the reader sees first. W4.C shipped as
 * "However, the evidence does provide…" because the sentence the "However"
 * contrasted with had already been deleted by INPUT_SENTENCE — the rule's own
 * optional `However,?` prefix belongs to the sentence being removed, not to the
 * one that inherits the opening position.
 */
const LEADING_CONNECTIVE = /^\s*(?:however|but|yet|still|additionally|also|moreover|furthermore|nevertheless|nonetheless|in\s+addition|that\s+said|even\s+so|on\s+the\s+other\s+hand|instead)\b[,:;]?\s+/i

/** Uppercase the first letter, leaving markdown and ids that start a line alone. */
function recapitalise(text) {
  return String(text).replace(/^([a-z])/, (_m, c) => c.toUpperCase())
}

/**
 * Drop a contrastive or additive opener that has nothing left to refer back to.
 *
 * Only at the very start of the answer. A connective in the middle still has
 * the preceding sentence to lean on, whatever we cut elsewhere, so this is the
 * one position where the word is certainly stranded.
 */
function stripLeadingConnective(text) {
  const s = String(text)
  if (!LEADING_CONNECTIVE.test(s)) return s
  return recapitalise(s.replace(LEADING_CONNECTIVE, ''))
}
...
export function stripHarnessFraming(text = '') {
  ...
  // wreckage, and this module does not edit prose it did not break.
  if (out !== before) out = stripLeadingConnective(out)
...
export function hasHarnessFraming(text = '') {
  ...
  return new RegExp(`${QUERY_NOUN}\\s+${NOT_RUN}`, 'i').test(s)
    ...
    || EVIDENCE_SUBJECT_RULES.some(([re]) => new RegExp(re.source, 'i').test(s))
    || LEADING_CONNECTIVE.test(s)
}
