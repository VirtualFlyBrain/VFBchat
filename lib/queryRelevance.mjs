...
export function queryRelevanceScore(question, digest, query, words = listQueryWords, kinds = null) {
  ...
  const semantics = querySemantics(query?.query_type || '')
  ...
  // A secondary kind (QUERY_SEMANTICS.carries) matches at full weight but is not
  // additive: a query that answers a question two ways is still one query, and
  // letting the weights stack would float ListAllAvailableImages above a query
  // whose PRIMARY kind is what was asked for.
  const kindMatch = wanted.has(kind) || (semantics.carries || []).some(k => wanted.has(k))
  return lexical + (kindMatch ? KIND_MATCH_WEIGHT : 0)
}
