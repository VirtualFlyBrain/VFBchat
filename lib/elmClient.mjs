...
export async function callStructured(o) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    const res = await postJson(`${baseUrl.replace(/\/$/, '')}${CHAT_COMPLETIONS}`, body, apiKey, attemptTimeoutMs, fetchImpl, signal)
...
// `signal` is the request's run signal: the client disconnecting, or the run
// deadline passing, must cancel an ELM call in flight, not merely stop the next
// one being started. A planner vote is three concurrent calls; an abandoned
// request used to leave all of them running.
async function postJson(url, body, apiKey, timeoutMs, fetchImpl, signal = null) {
