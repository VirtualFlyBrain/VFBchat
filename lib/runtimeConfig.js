...
// The site's own search index. virtualflybrain.org/search/ is client-side: it
// fetches /index.json (every page's title, description, section and body text,
// rebuilt with the site) and queries Solr for terms. The pages half is the
// documentation corpus the chat wants; terms it already gets from the MCP.
export function getReviewedDocsSiteIndexUrl() {
  return trimEnv('REVIEWED_DOCS_SITE_INDEX_URL') || 'https://www.virtualflybrain.org/index.json'
}
