...
export function collapseOntologyChains(rows = []) {
  ...
  for (const entry of sorted) {
    const g = groups.find(grp => {
      ...
      // The same connections carry the same synapses. On a small class the
      // counts alone coincide by chance — for KCg-s, "VP3 vPN" (9 pairs, 6
      // connected, 603 synapses) and "VP1m+VP2 lvPN 2" (9, 6, 46) are
      // unrelated types that were merged, and the top partner vanished into
      // an alias of a minor one. Weight has to agree as well.
      if (head.totalWeight || entry.r.totalWeight) {
        const tw = Math.min(head.totalWeight, entry.r.totalWeight) / Math.max(head.totalWeight, entry.r.totalWeight)
        if (!(tw >= SAME_CONNECTIONS)) return false
      }
      if (!head.connected || !entry.r.connected) return true
