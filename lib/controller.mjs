 ...
 *   action ∈ 'clarify' | 'resolve_terms' | 'run_step' | 'retrieve' | 'synthesise'
 *
 * There is no 'stop'. It was in this list and in a handler in runHarness, and
 * nextAction never returned it — so the handler was unreachable and gave false
 * assurance that the loop had a graceful dead stop. The real end of the loop is
 * `synthesise`, and the empty-answer guard in runRoleHarnessForRequest is what
 * covers a synthesis that produced nothing.
 */
