...
export function nextAction(ledger) {
  ...
  const unresolved = (ledger.termsToResolve || []).filter(name => !ledger.terms[name]?.id)
  ...
  // 6. Complete (or nothing left to do) → synthesise.
  if (isComplete(ledger) || ledger.plan.length > 0) {
    return { action: 'synthesise', reason: isComplete(ledger) ? 'complete' : 'no-more-steps' }
  }

  // 7. No plan at all → stop (shouldn't happen once a plan is set).
  return { action: 'stop', reason: 'no-plan' }
}
